...
export type XenesisConnectionChannelRuntimeSupport = 'implemented' | 'planned-adapter';
export type XenesisConnectionChannelRuntimeStatus = 'ready' | 'needs-setup' | 'planned-adapter' | 'unknown';

export interface XenesisConnectionChannelRuntimeTemplate {
  runtimeStatus: XenesisConnectionChannelRuntimeStatus;
  actionInboxKind: 'xenesis-channel-runtime-readiness';
  channel: string;
  displayName: string;
  adapter: string;
  runtimeSupport: XenesisConnectionChannelRuntimeSupport;
  primarySurface: string;
  setupSurface: string;
  reviewSurface: string;
...
export type XenesisConnectionToolRuntimeSupport = 'ready-template' | 'planned-oauth' | 'unknown';
export type XenesisConnectionToolRuntimeStatus = 'ready' | 'needs-setup' | 'planned-oauth' | 'unknown';

export interface XenesisConnectionToolRuntimeTemplate {
  runtimeStatus: XenesisConnectionToolRuntimeStatus;
...
export interface XenesisConnectionToolMcpOAuthTemplate {
  status: XenesisConnectionToolMcpOAuthStatus;
  actionInboxKind: 'xenesis-tool-mcp-oauth';
  tool: string;
  displayName: string;
  serverName: string;
  transport: XenesisConnectionMcpTemplate['transport'];
  authMode: 'oauth';
  runtimeSupport: XenesisConnectionToolMcpOAuthRuntimeSupport;
  authSurface: string;
  reviewSurface: string;
  credentialRefs: XenesisConnectionToolConnectorCredentialRef[];
  missingRequiredFields: string[];
...
export interface XenesisConnectionItem {
  ...
  channelRuntime?: XenesisConnectionChannelRuntimeTemplate;
...
function withXenesisToolMcpOAuthViewSection(
  view: XenesisConnectionToolViewTemplate | undefined,
  toolId: string,
  oauth: XenesisConnectionToolMcpOAuthTemplate | undefined,
): XenesisConnectionToolViewTemplate | undefined {
  if (!view || !oauth) return view;
  if (view.viewSections.some((section) => section.id === 'mcp-oauth')) return view;
  const oauthSection = toolViewSection({
    ...
    safetyBoundaries: ['MCP OAuth readiness view opens do not start OAuth flows, store tokens, or execute tools.'],
  });
  return {
    ...view,
    internalViews: uniqueStrings([...view.internalViews, 'mcp-oauth']),
    viewSections: [...view.viewSections, oauthSection],
    readPaths: uniqueStrings([...view.readPaths, ...oauthSection.readPaths]),
    controlPaths: uniqueStrings([...view.controlPaths, ...oauthSection.controlPaths]),
    diagnostics: uniqueStrings([...view.diagnostics, ...oauthSection.diagnostics]),
    safetyBoundaries: uniqueStrings([...view.safetyBoundaries, ...oauthSection.safetyBoundaries]),
  };
}
...
function channelRuntimeStatus(item: XenesisConnectionItem): XenesisConnectionChannelRuntimeStatus {
  if (item.messengerView?.runtimeSupport === 'planned' || item.supportLevel === 'planned') return 'planned-adapter';
  if (item.status === 'ready') return 'ready';
  if (item.status === 'needs-setup' || item.status === 'blocked' || item.status === 'disabled') return 'needs-setup';
  return 'unknown';
}

function channelRuntimeReadinessChecks(item: XenesisConnectionItem, plannedAdapter: boolean): string[] {
  if (plannedAdapter) {
    return uniqueStrings([
      'adapter-verified',
      'gateway-readback',
      'allowlist-reviewed',
      'pairing-state-readback',
      'profile-draft-reviewed',
...
function buildXenesisChannelRuntime(item: XenesisConnectionItem): XenesisConnectionChannelRuntimeTemplate | undefined {
  if (item.kind !== 'messenger') return undefined;
  const plannedAdapter = item.messengerView?.runtimeSupport === 'planned' || item.supportLevel === 'planned';
  const runtimeSupport: XenesisConnectionChannelRuntimeSupport = plannedAdapter ? 'planned-adapter' : 'implemented';
  const readinessChecks = channelRuntimeReadinessChecks(item, plannedAdapter);
  return {
    runtimeStatus: channelRuntimeStatus(item),
    actionInboxKind: 'xenesis-channel-runtime-readiness',
    channel: item.id,
    ...
    reviewSurface: 'Desk Action Inbox',
    gatewayRequirement: plannedAdapter
      ? 'planned adapter must be implemented and verified before runtime use'
      : 'gateway must be running and channel readiness must read back before delivery',
    readinessChecks,
    ...
    controlPaths: uniqueStrings([
      ...
      'xd.xenesis.messengers.views.open',
      'xd.xenesis.connections.open',
      ...(plannedAdapter ? [] : ['xd.xenesis.profiles.testChannel']),
    ...
    diagnostics: uniqueStrings([
      ...
      'gateway-status',
      'readback-verification',
      ...(plannedAdapter ? ['planned-adapter'] : ['implemented-adapter']),
      ...(item.messengerView?.diagnostics ?? []),
    ...
    blockedActions: uniqueStrings([
      ...
      'start gateway outside approved runtime path',
      ...
      ...(plannedAdapter
        ? [
            ...
            'pair accounts or devices from runtime readiness',
          ]
        : []),
    ...
    safetyBoundaries: uniqueStrings([
      plannedAdapter
        ? 'planned channel runtime readiness is review-only until the adapter is implemented, paired, allowlisted, and verified'
        : 'channel runtime readiness does not start gateways, send messages, mutate channel profiles, or store credentials',
      'runtime readiness opens and requests create/read local Desk surfaces only',
      'approval-gated profile draft apply and sanitized test-send paths remain separate',
...
function withXenesisConnectionChannelRuntime(item: XenesisConnectionItem): XenesisConnectionItem {
  const channelRuntime = buildXenesisChannelRuntime(item);
  if (!channelRuntime) return item;
  return { ...item, channelRuntime };
}

function withXenesisConnectionChannelRuntimes(
  sections: XenesisConnectionsStatus['sections'],
): XenesisConnectionsStatus['sections'] {
  return Object.fromEntries(
    Object.entries(sections).map(([id, section]) => [
      id,
      { ...section, items: section.items.map((item) => withXenesisConnectionChannelRuntime(item)) },
    ]),
...
function buildXenesisToolMcpOAuth(item: XenesisConnectionItem): XenesisConnectionToolMcpOAuthTemplate | undefined {
  if (item.kind !== 'tool') return undefined;
  if (item.toolConnector?.authMode !== 'oauth') return undefined;
  if (item.mcpTemplate?.auth !== 'oauth') return undefined;

  const scopes = uniqueStrings(item.toolConnector.dataScopes);
  const missingRequiredFields = item.toolConnector.credentialRefs
    .filter((credential) => credential.required && credential.state === 'missing')
    .map((credential) => credential.ref);
  const status: XenesisConnectionToolMcpOAuthStatus =
    missingRequiredFields.length > 0 ? 'missing-required-field' : 'ready-template';
  ...
  return {
    ...
    reviewSurface: 'Desk Action Inbox',
    credentialRefs: item.toolConnector.credentialRefs.map((credential) => ({ ...credential })),
    missingRequiredFields,
    scopes,
    ...
    diagnostics: uniqueStrings([
      ...
      'template-snippet',
      'cr-readback',
      ...missingRequiredFields.map((field) => `missing:${field}`),
      ...(item.toolConnector.diagnostics ?? []),
    ]),
    blockedActions: [
      'start OAuth flow',
...
function toolRuntimeSupport(item: XenesisConnectionItem): XenesisConnectionToolRuntimeSupport {
  if (
  ...
  if (item.toolConnector || item.toolInstallPlan || item.toolUserStory) return 'ready-template';
  return 'unknown';
}

function toolRuntimeStatus(
  support: XenesisConnectionToolRuntimeSupport,
  credentialState: XenesisConnectionToolConnectorCredentialState,
): XenesisConnectionToolRuntimeStatus {
  if (support === 'planned-oauth') return 'planned-oauth';
  if (support === 'unknown') return 'unknown';
  return credentialState === 'configured' || credentialState === 'not-required' ? 'ready' : 'needs-setup';
}

function buildXenesisToolRuntime(item: XenesisConnectionItem): XenesisConnectionToolRuntimeTemplate | undefined {
  if (item.kind !== 'tool') return undefined;

  const support = toolRuntimeSupport(item);
  const credentialState = item.toolConnector?.credentialState ?? (support === 'planned-oauth' ? 'planned' : 'missing');
  const requiredEnv = uniqueStrings([
    ...(item.requiredEnv ?? []),
    ...(item.toolInstallPlan?.requiredEnv ?? []),
    ...(item.mcpInstallDraft?.requiredEnv ?? []),
    ...(item.toolConnector?.credentialRefs
      .filter((credential) => credential.required && credential.source === 'env')
      .map((credential) => credential.ref) ?? []),
  ]);
  const missingEnv = uniqueStrings([
    ...(item.missingEnv ?? []),
    ...(item.mcpInstallDraft?.missingEnv ?? []),
    ...(item.toolConnector?.credentialRefs
      .filter((credential) => credential.required && credential.source === 'env' && credential.state === 'missing')
      .map((credential) => credential.ref) ?? []),
  ]);
  const readbackChecks = uniqueStrings([
  ...
  const plannedOauth = support === 'planned-oauth';

  return {
    runtimeStatus: toolRuntimeStatus(support, credentialState),
    actionInboxKind: 'xenesis-tool-runtime-readiness',
    tool: item.id,
    displayName: item.label,
    runtimeSupport: support,
    authMode: item.toolConnector?.authMode ?? item.toolSetup?.authMode ?? 'none',
    installSurface:
      item.toolInstallPlan?.installSurface ??
    ...
    runtimeSurface: plannedOauth ? 'planned OAuth MCP runtime' : 'provider MCP runtime',
    reviewSurface: 'Desk Action Inbox',
    credentialState,
    requiredEnv,
    missingEnv,
    ...
    blockedActions: uniqueStrings([
      'execute provider tools before runtime readback',
      'execute provider tools',
      'install MCP servers',
      'write MCP config',
      'store credentials',
      ...(plannedOauth ? ['complete OAuth', 'store OAuth tokens'] : []),
    ...
    safetyBoundaries: uniqueStrings([
      'tool runtime readiness is review-only',
      'tool runtime readiness does not execute provider tools, install MCP servers, write MCP config, store credentials, complete OAuth, store tokens, or mutate external systems',
      'credential values and OAuth tokens are never returned',
      ...(item.toolConnector?.safetyBoundaries ?? []),
      ...(item.toolInstallPlan?.safetyBoundaries ?? []),
      ...(item.toolActionCatalog?.safetyBoundaries ?? []),
  ...
  return [
    ...
    ...(item.toolRuntime
      ? [
          toolSetupPlanStep({
            id: 'tool-runtime',
            label: 'Read tool runtime readiness',
            kind: 'read',
            crPath: 'xd.xenesis.tools.runtime.status',
            itemId: item.id,
            expectedState: `${item.label} runtime support, credential state, and readback checks are visible before provider tool execution.`,
            verifyWith: item.toolRuntime.diagnostics,
            safetyBoundary: 'tool runtime readiness reads do not execute provider tools or mutate external systems',
          }),
        ]
      : []),
    ...
    ...(item.toolMcpOAuth
      ? [
          toolSetupPlanStep({
            id: 'mcp-oauth',
            label: 'Review MCP OAuth readiness',
            kind: 'request',
            crPath: 'xd.xenesis.tools.mcpOAuth.request',
            itemId: item.id,
            expectedState: `${item.label} MCP OAuth runtime readiness can be reviewed before any OAuth browser flow or provider tool execution.`,
            verifyWith: item.toolMcpOAuth.diagnostics,
            safetyBoundary: 'MCP OAuth readiness review does not start OAuth, store tokens, or execute tools',
          }),
        ]
      : []),
    ...(item.toolOAuthDraft
  ...
  return [
    ...
    ...(item.channelRuntime
      ? [
          channelSetupPlanStep({
            id: 'channel-runtime',
            label: 'Read channel runtime readiness',
            kind: 'read',
            crPath: 'xd.xenesis.channels.runtime.status',
            args: channelArgs,
            expectedState: `${item.label} channel runtime support, gateway requirement, readiness checks, and blocked actions are visible before delivery.`,
            verifyWith: [...item.channelRuntime.readinessChecks, ...item.channelRuntime.diagnostics],
            safetyBoundary:
              'channel runtime reads do not start gateways, pair accounts, mutate profiles, or send messages',
          }),
        ]
      : []),
...
function buildXenesisChannelSetupPlan(
  ...
  return {
    ...
    diagnostics: uniqueStrings([
      ...
      ...(item.messengerView?.diagnostics ?? []),
      ...(item.channelRuntime?.diagnostics ?? []),
      ...(item.channelRuntime?.readinessChecks ?? []),
      ...(item.channelTemplate?.routing?.diagnostics ?? []),
      ...(item.channelTemplate?.safety?.troubleshooting ?? []),
      ...(item.channelTemplate?.accessGroups?.diagnostics ?? []),
      ...(item.channelTemplate?.pairing?.validationChecks ?? []),
    ...
    blockedActions: uniqueStrings([
      ...
      ...(plannedAdapter
        ...
        : []),
      ...(item.channelRuntime?.blockedActions ?? []),
      ...(item.channelProfileDraft?.blockedActions ?? []),
    ]),
    safetyBoundaries: uniqueStrings([
      ...
      ...(item.messengerView?.safetyBoundaries ?? []),
      ...(item.channelRuntime?.safetyBoundaries ?? []),
      ...(item.channelTemplate?.safetyControls ?? []),
      ...(item.channelTemplate?.safety?.safetyBoundaries ?? []),
      ...(item.channelTemplate?.accessGroups?.safetyBoundaries ?? []),
      ...(item.channelTemplate?.pairing?.safetyBoundaries ?? []),
...
function diagnosticRunbookSetupSurface(item: XenesisConnectionItem): string {
  return (
    ...
    item.toolInstallPlan?.setupSurface ??
    item.mcpInstallDraft?.installSurface ??
    item.channelRuntime?.setupSurface ??
    item.channelProfileDraft?.setupSurface ??
    item.messengerView?.setupSurface ??
    item.channelTemplate?.pairing?.setupSurface ??
...
function diagnosticRunbookPrimarySurface(item: XenesisConnectionItem): string {
  return (
    ...
    (item.channelRuntime ? 'Settings > Xenesis Agent > Connections' : undefined) ??
...
function buildXenesisConnectionDiagnosticRunbook(
  ...
  if (item.toolRuntime) {
    steps.push(
      diagnosticRunbookStep({
        id: 'tool-runtime',
        label: 'Tool runtime readiness',
        expectedState:
          'Generic external tool runtime support, credential state, readback checks, and blocked actions are visible before provider tool execution.',
        readPaths: ['xd.xenesis.tools.runtime.status', ...item.toolRuntime.readPaths],
        controlPaths: item.toolRuntime.controlPaths,
        diagnostics: [
          ...item.toolRuntime.requiredEnv,
          ...item.toolRuntime.missingEnv,
  ...
  if (item.toolMcpOAuth) {
    steps.push(
      diagnosticRunbookStep({
        id: 'tool-mcp-oauth',
        label: 'Tool MCP OAuth readiness',
        expectedState:
          'Review-only MCP OAuth runtime readiness is visible before OAuth starts, tokens are stored, or provider tools execute.',
        readPaths: ['xd.xenesis.tools.mcpOAuth.status', ...item.toolMcpOAuth.readPaths],
        controlPaths: item.toolMcpOAuth.controlPaths,
        diagnostics: [
          ...item.toolMcpOAuth.missingRequiredFields,
          ...item.toolMcpOAuth.scopes,
          ...item.toolMcpOAuth.diagnostics,
        ],
      }),
    );
  ...
  if (item.channelRuntime) {
    steps.push(
      diagnosticRunbookStep({
        id: 'channel-runtime',
        label: 'Channel runtime readiness',
        expectedState:
          'Runtime support, gateway requirement, readiness checks, and blocked actions are visible before delivery.',
        readPaths: ['xd.xenesis.channels.runtime.status', ...item.channelRuntime.readPaths],
...
function setupRequestStepItems(item: XenesisConnectionItem): string[] {
  return uniqueStrings([
    ...
    ...(item.toolMcpOAuth?.missingRequiredFields.map((field) => `review MCP OAuth field: ${field}`) ?? []),
    ...(item.toolMcpOAuth?.scopes.map((scope) => `review MCP OAuth scope: ${scope}`) ?? []),
    ...
    ...(item.channelRuntime?.readinessChecks.map((check) => `verify channel runtime readiness: ${check}`) ?? []),
    ...(item.channelRuntime?.blockedActions.map((action) => `confirm channel runtime blocks: ${action}`) ?? []),
...
function toolConnectionItems(env: Record<string, string | undefined> = {}): XenesisConnectionItem[] {
  return TOOL_CONNECTIONS.map((item) => {
    ...
    const toolRuntime = buildXenesisToolRuntime({
      ...withMcpInstallDraft,
      toolMcpOAuth,
      toolOAuthDraft,
      toolOAuthRuntime,
