...
test('capability approval request creates Action Inbox record shape', () => {
  const requestArgs = { id: 'notion' };
  const approvalKey = createCapabilityApprovalAllowKey({
    path: REQUEST.path,
    args: requestArgs,
    source: 'xenesis',
  });
  const approvalKeySuffix = approvalKey.split(':').pop();
  ...
  assert.equal(item.id, `capability-xenesis-xd.xenesis.connections.setupRequests.request-${approvalKeySuffix}`);
  ...
  assert.equal(item.approvalSessionKey, approvalKey);
