...
function xenesisRuntimeInventoryActionFromNaturalText(value: string): XenesisDeskActionRequest | null {
...
export function planXenesisDeskNaturalLanguageActions(text: string): XenesisDeskNaturalLanguagePlan {
  ...
  const xenesisRuntimeInventoryAction = xenesisRuntimeInventoryActionFromNaturalText(value);
...
export function buildXenesisDeskControlPromptHint(): string {
  return [
    ...
    '- Use `xd.xenesis.diagnostics`, `xd.xenesis.reports.list`, `xd.xenesis.tasks.list`, and `xd.xenesis.agents.list` to inspect runtime diagnostics, verification reports, task inventory, and registered Agent panes before submitting prompts or mutating runtime state.',
    ...
    '- Use `xd.xenesis.runs.cancel` only for explicit user requests to cancel the active Xenesis runtime request, and `xd.xenesis.sessions.reset` only for explicit user requests to reset the active Xenesis conversation/session.',
    '- Use `xd.xenesis.tools.setup.status`, `xd.xenesis.tools.views.status`, and `xd.xenesis.tools.views.open` to inspect or open internal Desk tool setup/readiness surfaces.',
    ...
    '- Use `xd.xenesis.channels.routing.status`, `xd.xenesis.channels.safety.status`, `xd.xenesis.messengers.views.status`, and `xd.xenesis.messengers.views.open` before testing or changing external messenger setup.',
