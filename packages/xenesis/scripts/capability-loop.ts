...
import { SqliteAgentTaskStore, type AgentTaskExecutor } from "../src/orchestration/index.js";
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--self-review-cooldown-ms") parsed.selfReviewCooldownMs = nonNegativeInteger(next(), "--self-review-cooldown-ms");
    else if (arg === "--failure-pattern-cooldown-ms") parsed.failurePatternCooldownMs = nonNegativeInteger(next(), "--failure-pattern-cooldown-ms");
    else if (arg === "--max-next-tasks") parsed.maxNextTasks = positiveInteger(next(), "--max-next-tasks");
    else if (arg === "--max-policy-impact-reworks") parsed.maxPolicyImpactReworkAttempts = nonNegativeInteger(next(), "--max-policy-impact-reworks");
    else if (arg === "--max-cycle-minutes") parsed.maxCycleMinutes = positiveNumber(next(), "--max-cycle-minutes");
    else if (arg === "--max-cycle-input-tokens") parsed.maxCycleInputTokens = positiveInteger(next(), "--max-cycle-input-tokens");
    else if (arg === "--max-cycle-output-tokens") parsed.maxCycleOutputTokens = positiveInteger(next(), "--max-cycle-output-tokens");
    else if (arg === "--max-cycle-total-tokens") parsed.maxCycleTotalTokens = positiveInteger(next(), "--max-cycle-total-tokens");
    else if (arg === "--max-cycle-cost-usd") parsed.maxCycleCostUsd = positiveNumber(next(), "--max-cycle-cost-usd");
    else if (arg === "--input-token-usd-per-1m") parsed.inputTokenUsdPer1m = positiveNumber(next(), "--input-token-usd-per-1m");
    else if (arg === "--output-token-usd-per-1m") parsed.outputTokenUsdPer1m = positiveNumber(next(), "--output-token-usd-per-1m");
    ...
    else if (arg === "--auto-follow-up-after-failures") parsed.autoFollowUpAfterFailures = positiveInteger(next(), "--auto-follow-up-after-failures");
...
function maxElapsedMs(parsed: ParsedArgs) {
  return parsed.maxCycleMinutes === undefined
    ? undefined
    : Math.ceil(parsed.maxCycleMinutes * 60_000);
}
...
function addRunUsage(target: CapabilityLoopUsageSummary, runs: Array<{ agentTask: { usage?: CapabilityLoopUsageSummary } }>) {
...
async function readRunReports(
  dir: string,
  maxItems: number
): Promise<CapabilityImprovementRunReportInput[]> {
  const reports = await readRunReportObjects(dir, maxItems);
  return reports
    .map(({ report }) => report as CapabilityImprovementRunReportInput)
    .filter((report): report is CapabilityImprovementRunReportInput => Boolean(
      report.sessionId &&
      report.createdAt &&
      report.selfReview &&
...
async function readFailedRunReportLearningEntries(
  runReportsDir: string,
  sessionsDir: string,
  maxItems: number
) {
  const records = await readRunReportObjects(runReportsDir, maxItems);
  return await Promise.all(records.map(async ({ fileName, report }) => {
...
async function main() {
  ...
  const backlogPath = resolve(parsed.backlog ?? resolve(resolveXenesisHome(), "reports", "capability-scenario-backlog.json"));
  ...
  for (let cycle = 1; cycle <= parsed.cycleLimit; cycle += 1) {
    ...
    if (!preTaskSafetyStopReason && parsed.autoImprove && backlog) {
      ...
      const existingTaskBacklog = improvement?.taskBacklog ?? await readCapabilityImprovementTaskBacklog(improvementTasksPath);
      const existingTaskResults = improvement?.taskResults ?? await readCapabilityImprovementTaskResults(improvementResultsPath);
    ...
    if (!preTaskSafetyStopReason && parsed.selectNext && improvement) {
      ...
      const selection = cycleNextLimit > 0
        ? selectCapabilityLoopNextTasks({
          report: loopReport,
          taskBacklog: improvement.taskBacklog,
      ...
      nextTasksSummary.readinessSteering.push(...selection.selected
  ...
  const safety = {
    ...
    selfReviewCooldownMs: parsed.selfReviewCooldownMs,
    failurePatternCooldownMs: parsed.failurePatternCooldownMs,
    ...(parsed.autoFollowUpAfterFailures !== undefined ? { autoFollowUpAfterFailures: parsed.autoFollowUpAfterFailures } : {}),
  ...
  if (parsed.summaryOut) {
    ...
    const lines = [
      ...
      ...(failureTypes.length > 0
        ? failureTypes.slice(0, 12).map((failure) => `- ${failure}`)
        : ["none"]),
      "",
      "## Next Tasks",
      ...
      ...(summary.nextTasks?.selected.length
        ? ["", ...summary.nextTasks.selected.map((taskId) => `- ${taskId}`)]
        : []),
      "",
      ...
      ...(previewCandidates.length > 0
        ? [
          ...
          "| --- | --- | --- | --- | --- |",
          ...previewCandidates.map((task, index) => [
            index + 1,
