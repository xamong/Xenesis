...
function run(command, args, options = {}) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error([
      `${commandName(command)} ${args.join(" ")} failed with exit code ${result.status}.`,
      output
    ].filter(Boolean).join("\n"));
  }
