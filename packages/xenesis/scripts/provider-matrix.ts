...
function runProvider(target: ProviderMatrixTarget, options: {
  scenarioPath: string;
  timeoutMs: number;
  reportDir: string;
...
async function main() {
  ...
  const reportPath = resolve(parsed.report ?? resolve(xenesisHome, "reports", `provider-matrix-${stamp(new Date())}.json`));
  ...
  try {
    ...
    if (parsed.json) console.log(JSON.stringify(report, null, 2));
      ...
      console.log(`provider-matrix: passed ${report.summary.passed}/${report.summary.total} skipped ${report.summary.skipped}`);
      console.log(`provider-matrix: usage totalTokens=${report.usage.totalTokens} measured=${report.usage.measuredProviders.join(",") || "none"}`);
    }
