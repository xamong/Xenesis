...
import { type BrowserDriver, type BrowserSnapshot, PlaywrightBrowserDriver } from './browserDriver.js';
...
function visibleCanvasElements(snapshot: BrowserSnapshot) {
  return (snapshot.canvases ?? []).filter(
    (canvas) =>
      canvas.visible &&
      ((canvas.width > 0 && canvas.height > 0) || (canvas.clientWidth > 0 && canvas.clientHeight > 0)),
  );
}
...
function createVisualVerification(snapshot: BrowserSnapshot): AppE2EVisualVerification {
  ...
  const screenshotFallbackReason: ScreenshotFallbackReason =
    directStatus === 'observed'
      ? 'not_needed'
  ...
  const method: VisualVerificationMethod =
    directStatus === 'observed'
      ? 'direct_canvas_pixels'
...
function looksLikeHttpErrorPage(snapshot: BrowserSnapshot) {
  const corpus = `${snapshot.title}\n${snapshot.text}`;
  return /HTTP\s*(?:오류|Error)\s*[45]\d\d|IIS\s+\d+(?:\.\d+)?[^.\n]*[45]\d\d|[45]\d\d(?:\.\d+)?\s*-\s*(?:Not Found|Forbidden|Internal Server Error|Bad Gateway|Service Unavailable)/i.test(
    corpus,
  );
}
...
function looksLikeDefaultServerPage(snapshot: BrowserSnapshot) {
  const corpus = `${snapshot.title}\n${snapshot.text}`;
  return /IIS\s+Windows\s+Server|Apache2?\s+(?:Ubuntu\s+)?Default\s+Page|Welcome\s+to\s+nginx|nginx\s+welcome|It\s+works!/i.test(
    corpus,
  );
}
...
function isAmbiguousVisualVerification(visualVerification: AppE2EVisualVerification) {
  return visualVerification.method !== 'direct_canvas_pixels' || visualVerification.directCanvasPixelStatus === 'blank';
}
    ...
    expectedMissing.length > 0
      ...
      : genericExpectedMissing.length > 0
        ...
        : canvasExpectedMissing.length > 0
          ...
          : input.expectedText.length > 0
            ? 'expectedText: all present'
            : 'expectedText: none configured',
  ];
...
export function createAppE2ECheckTool(options: AppE2ECheckOptions): Tool<AppE2ECheckInput, AppE2ECheckData> {
  return {
    name: 'app_e2e_check',
    description:
      'Open a rendered app page and check UI quality signals such as expected text, broken undefined values, body text, and interactive controls.',
    ...
    async run(input) {
      ...
      try {
        ...
        // stays scalar text.
        const attachments = isAmbiguousVisualVerification(visualVerification) ? verificationAttachments(snapshot) : [];
