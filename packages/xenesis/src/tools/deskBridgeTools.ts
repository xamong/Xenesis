...
function resolveXvCommandRoute(command: string, approved: boolean): XvCommandRoute {
  ...
  throw new Error(
    `Unsupported /xd command: ${verbRaw}. Use /xd capabilities or /xd call <path> <json> for lower-level CR access.`,
  );
}
...
function formatCapabilityCallResult(payload: BridgePayload, fallbackPath: string) {
  if (payload.approvalRequired) {
    const item =
      payload.actionInboxItem && typeof payload.actionInboxItem === 'object'
...
function formatCapabilities(payload: BridgePayload) {
  ...
  const lines = capabilities
    .map((capability) => {
      ...
      const flags = [typed.readable === true ? 'readable' : '', typed.callable === true ? 'callable' : ''].filter(
        Boolean,
      );
...
function formatCapabilityDescription(payload: BridgePayload, fallbackPath: string) {
  const capability =
    payload.capability && typeof payload.capability === 'object' ? (payload.capability as Record<string, unknown>) : {};
  return [
...
export const deskStateTool: Tool<z.infer<typeof bridgeReadInput>> = {
  name: 'desk_state',
  description:
    'Read the current Xenesis Desk bridge state: terminals, panels, open files, diagnostics, and renderer state.',
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk state request failed.'), data: payload };
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false)
        return {
          ok: false,
          content: String(payload.error ?? 'Xenesis Desk active context request failed.'),
          data: payload,
        };
...
export const deskBrowserListTool: Tool<z.infer<typeof deskBrowserListInput>> = {
  ...
  description:
    'Read visible Xenesis Desk browser panes, including count, titles, URLs, and active state. Use for natural-language browser tab status questions.',
  ...
  async run(input, context) {
    try {
      ...
      if (payload.approvalRequired)
        return { ok: true, content: 'Browser pane list requires Desk approval.', data: payload };
      if (payload.ok === false)
        return { ok: false, content: String(payload.error ?? 'Browser pane list failed.'), data: payload };
...
export const deskExplorerStateTool: Tool<z.infer<typeof deskExplorerStateInput>> = {
  ...
  description:
    'Read local Xenesis Desk file explorer state, including whether it is open, its root, and selected path.',
  ...
  async run(input, context) {
    try {
      ...
      if (payload.approvalRequired)
        return { ok: true, content: 'File explorer state requires Desk approval.', data: payload };
      if (payload.ok === false)
        return { ok: false, content: String(payload.error ?? 'File explorer state failed.'), data: payload };
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false)
        return {
          ok: false,
          content: String(payload.error ?? 'Xenesis Desk capability request failed.'),
          data: payload,
        };
      return {
        ok: true,
...
export const deskCallCapabilityTool: Tool<z.infer<typeof callCapabilityInput>> = {
  ...
  description:
    'Call a registered Xenesis Desk bridge capability by path. Control/write/execute capabilities may require Desk approval. Use xd.apps.* for visible external desktop app control, and prefer registered appId values such as notepad over arbitrary executable paths.',
  ...
  async run(input, context) {
    try {
      ...
      const payload = await callDeskBridge(
        context,
        '/capabilities/call',
        {
          path: input.path,
      ...
      if (payload.ok === false)
        return {
          ok: false,
          content: String(payload.error ?? `Xenesis Desk capability call failed: ${input.path}`),
          data: payload,
        };
      return {
        ok: true,
...
export const deskXvCommandTool: Tool<z.infer<typeof deskXvCommandInput>> = {
  ...
  description:
    'Route a concise /xd command through the Xenesis Desk bridge and Capability Registry. Supports status, context, actions, logs, commands, run, tail, stop, open, xcon, validate, capabilities, call, and xd.apps.* external desktop app control such as notepad.',
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired) {
        return {
          ok: false,
          content: String(payload.error ?? `Xenesis Desk /xd command failed: ${route.label}`),
          data: payload,
        };
      }
      return {
...
export const deskOpenFileTool: Tool<z.infer<typeof deskOpenFileInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk file open failed.'), data: payload };
...
export const deskTerminalRunTool: Tool<z.infer<typeof deskTerminalRunInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk terminal command failed.'), data: payload };
...
export const deskTerminalRunAndWaitTool: Tool<z.infer<typeof deskTerminalRunAndWaitInput>> = {
  ...
  description:
    'Run a one-shot command through Xenesis Desk and return exit status plus captured output. Prefer this when the task needs command results, not a visible interactive terminal.',
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return {
          ok: false,
          content: String(payload.error ?? 'Xenesis Desk terminal run-and-wait failed.'),
          data: payload,
        };
...
export const deskSubagentStartTool: Tool<z.infer<typeof deskSubagentStartInput>> = {
  ...
  description:
    'Start delegated work as a Desk-visible subagent in its own Xenesis Desk terminal. Prefer this over invisible background delegation when the user wants to watch each subagent separately.',
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk subagent terminal failed.'), data: payload };
      return {
        ok: true,
...
export const deskSubagentListTool: Tool<z.infer<typeof deskSubagentListInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk terminal list failed.'), data: payload };
...
export const deskSubagentTailTool: Tool<z.infer<typeof deskTerminalTailInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk subagent tail failed.'), data: payload };
      return { ok: true, content: jsonContent(payload.result ?? payload), data: payload };
    } catch (error) {
      return errorResult(error);
    }
...
export const deskSubagentStopTool: Tool<z.infer<typeof deskTerminalStopInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk subagent stop failed.'), data: payload };
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return {
          ok: false,
          content: String(payload.error ?? 'Xenesis Desk command palette list failed.'),
          data: payload,
        };
      return { ok: true, content: jsonContent(payload.result ?? payload), data: payload };
    } catch (error) {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return {
          ok: false,
          content: String(payload.error ?? 'Xenesis Desk command palette command failed.'),
          data: payload,
        };
...
export const deskCreateXconMarkdownTool: Tool<z.infer<typeof deskCreateXconMarkdownInput>> = {
  ...
  description:
    'Create a rendered XCON/SKETCH Markdown artifact inside Xenesis Desk. Do not use this for normal workspace .md file writes; use write, patch, or edit instead.',
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return {
          ok: false,
          content: String(payload.error ?? 'Xenesis Desk XCON Markdown creation failed.'),
          data: payload,
        };
      return {
        ok: true,
        content: formatCapabilityCallResult(payload, 'xd.artifacts.xconMarkdown.create'),
        data: payload,
      };
    } catch (error) {
      return errorResult(error);
...
export const deskExportXconPdfTool: Tool<z.infer<typeof deskExportXconPdfInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk XCON PDF export failed.'), data: payload };
      return {
        ok: true,
        content: formatCapabilityCallResult(payload, 'xd.artifacts.xconMarkdown.exportPdf'),
        data: payload,
      };
    } catch (error) {
      return errorResult(error);
...
export const deskTerminalTailTool: Tool<z.infer<typeof deskTerminalTailInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk terminal tail failed.'), data: payload };
      return { ok: true, content: jsonContent(payload.result ?? payload), data: payload };
    } catch (error) {
      return errorResult(error);
    }
...
export const deskTerminalStopTool: Tool<z.infer<typeof deskTerminalStopInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk terminal stop failed.'), data: payload };
      return { ok: true, content: formatCapabilityCallResult(payload, 'xd.terminals.stop'), data: payload };
    } catch (error) {
      return errorResult(error);
    }
...
export const deskContextActionsTool: Tool<z.infer<typeof deskContextActionsInput>> = {
  ...
  async run(input, context) {
    try {
      const payload = await callCapabilityPath(context, 'xd.context.actions', {}, false, input.timeoutMs ?? 5000);
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk context actions failed.'), data: payload };
      return { ok: true, content: jsonContent(payload.result ?? payload), data: payload };
    } catch (error) {
      return errorResult(error);
    }
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return {
          ok: false,
          content: String(payload.error ?? 'Xenesis Desk recent diagnostics failed.'),
          data: payload,
        };
      return { ok: true, content: jsonContent(payload.result ?? payload), data: payload };
    } catch (error) {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return {
          ok: false,
          content: String(payload.error ?? 'Xenesis Desk Playwright snapshot failed.'),
          data: payload,
        };
...
export const deskPlaywrightRunTool: Tool<z.infer<typeof deskPlaywrightRunInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk Playwright run failed.'), data: payload };
...
export const deskSafeFilePreviewTool: Tool<z.infer<typeof deskSafeFilePreviewInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk safe file preview failed.'), data: payload };
      return { ok: true, content: jsonContent(payload.result ?? payload), data: payload };
    } catch (error) {
      return errorResult(error);
    }
...
export const deskSafeFileApplyTool: Tool<z.infer<typeof deskSafeFileApplyInput>> = {
  ...
  async run(input, context) {
    try {
      ...
      if (payload.ok === false && !payload.approvalRequired)
        return { ok: false, content: String(payload.error ?? 'Xenesis Desk safe file apply failed.'), data: payload };
