import { z } from 'zod';
import { type AgentTask, SqliteAgentMessageStore, SqliteAgentTaskStore } from '../orchestration/index.js';
...
function matchesRecipient(task: AgentTask, to: string) {
  return task.id === to || metadataName(task) === to || task.label === to || task.subagent === to;
}
...
async function queueToActiveAgent(target: AgentTask, input: SendMessageInput, context: ToolContext) {
...
async function broadcastMessage(store: SqliteAgentTaskStore, input: SendMessageInput, context: ToolContext) {
