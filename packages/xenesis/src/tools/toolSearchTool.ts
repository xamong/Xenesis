...
const toolHints: Record<string, string[]> = {
  ...
  app_e2e_check: ["app", "e2e", "check", "browser", "render", "rendered", "ui", "quality", "undefined", "nan", "broken", "client", "page", "smoke"],
...
function parseToolName(name: string) {
  if (name.startsWith("mcp__")) {
    ...
    const parts = withoutPrefix.split("__").flatMap((part) => part.split("_")).filter(Boolean);
    return {
      parts,
...
function searchDeferredTools(registry: ToolRegistry, query: string, maxResults: number) {
  ...
  if ((queryLower.startsWith("mcp__") && queryLower.length > 5) || (queryLower.startsWith("mcp_") && queryLower.length > 4)) {
...
function penalizedByQuery(query: string, toolName: string) {
  ...
  return normalized.includes(`without ${toolName}`) ||
    ...
    async run(input) {
      ...
      if (deferredTools.length > 0) {
        ...
        if (explicitDeferredSearch || hasStrongDeferredMatch(registry, search.matches, input.query)) {
          ...
          return {
            ok: true,
            content: search.matches.length > 0
