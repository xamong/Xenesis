import { buildMarkOverlayScript, removeMarkOverlayScript, type MarkInput } from "../core/surface/som.js";
    ...
    try {
      ...
      const stats = await page.evaluate(async (src) => {
        ...
        return {
          ...
          maxChannelDelta: opaqueSamples > 0
  ...
  private async captureOnce(): Promise<BrowserSnapshot> {
    ...
    for (const locator of canvasLocators.slice(0, MAX_CANVASES)) {
      ...
      const info = await locator.evaluate((node) => {
        ...
        const style = typeof browserGlobal.getComputedStyle === "function"
          ? browserGlobal.getComputedStyle(canvas)
          : undefined;
        ...
        try {
          ...
          if (context && width > 0 && height > 0) {
            ...
            const sampleRects = [
              [0, 0],
              [Math.max(0, Math.floor(width / 2) - Math.floor(sampleWidth / 2)), Math.max(0, Math.floor(height / 2) - Math.floor(sampleHeight / 2))],
