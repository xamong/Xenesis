...
async function exitPlanMode(input: ExitPlanModeInput, context: ToolContext) {
  ...
  if (!current || current.mode !== "plan") {
    return {
      ok: false,
      content: "You are not in plan mode. This tool is only for exiting plan mode after writing a plan. If your plan was already approved, continue with implementation."
    };
  }
