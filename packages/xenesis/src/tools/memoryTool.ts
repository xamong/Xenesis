import { z } from "zod";
import type { MemoryEvidenceRecord, MemoryLedgerEvent, MemoryProposal, MemoryRecord, MemoryStore, MemoryWriteContext, MemoryWriteResult } from "../extensions/index.js";
        ...
        if (isMemoryLedger(target)) {
          if (input.action === "save") {
            ...
            if (result.status === "accepted" && result.record) {
              return { ok: true, content: `memory: saved ${result.record.id}`, data: redactWriteResultForModel(result) };
            }
            return { ok: true, content: `memory: proposed ${result.proposal?.id ?? "(unknown)"}`, data: redactWriteResultForModel(result) };
          }
          ...
          if (input.action === "evidence") {
            const records = (input.id
              ? [await target.getEvidence(input.id)].filter((record): record is MemoryEvidenceRecord => Boolean(record))
              : await target.listEvidence()).map(redactEvidenceForModel);
            return {
              ok: true,
          ...
          if (input.action === "search") {
            const records = (await target.searchRecords({ query: requireQuery(input), limit: 10 })).map(redactRecordForModel);
            return {
              ok: true,
