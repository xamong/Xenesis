...
const configOpenAIInputSchema = z.object({
  setting: z.string().min(1),
...
const supportedSettings: Record<string, SupportedConfigSetting> = {
  ...
  "context.stripOldImages": {
    type: "boolean",
    description: "Strip base64 image attachments from messages outside the recent window before sending to the provider."
  },
...
function coerceInteger(setting: string, value: string | boolean | number, minimum: number) {
  if (typeof value === "boolean") {
    return { ok: false as const, error: `${setting} requires ${minimum > 0 ? "a positive" : "a non-negative"} integer.` };
  }
...
function resultFromData(data: ConfigOutput) {
  if (data.success) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(data, "effectiveValue") &&
