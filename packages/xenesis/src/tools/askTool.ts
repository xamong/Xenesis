...
const referenceAskInput = z
  .object({
    questions: z.array(askQuestionInput).min(1).max(4),
    question: nullableOptionalString,
    answers: z.preprocess((value) => (value === null ? undefined : value), z.record(z.string()).optional()),
    annotations: z.preprocess((value) => (value === null ? undefined : value), z.record(askAnnotationInput).optional()),
  ...
  async run(input, context) {
    if (isReferenceAskInput(input)) {
      ...
      return {
        ok: true,
        content:
          Object.keys(data.answers).length > 0 ? resultBlockContent(data) : `User input required: ${combinedQuestion}`,
        data,
      };
    }
