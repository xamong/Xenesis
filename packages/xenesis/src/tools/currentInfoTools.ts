...
const weatherForecastInput = z.object({
  location: z.string().min(1).nullable().optional(),
...
const sportsInput = z.object({
  league: z.enum(["nba", "nfl", "mlb", "nhl", "epl", "fifa_world_cup", "fifa_club_world_cup", "fifa_world_cup_qualifying"]),
...
function canonicalWeatherLocation(location: string) {
  ...
  const withoutCountry = trimmed
    .replace(/\s*,\s*(south korea|republic of korea|korea|대한민국|한국)\s*$/i, "")
    .trim();
  ...
  async run(input, context) {
    ...
    return {
      ...
      content: [
        ...
        current.apparent_temperature !== undefined ? `- Feels like: ${current.apparent_temperature}${apparentUnit}` : undefined,
        current.relative_humidity_2m !== undefined ? `- Humidity: ${current.relative_humidity_2m}${humidityUnit}` : undefined,
        current.precipitation !== undefined ? `- Precipitation: ${current.precipitation}${precipitationUnit}` : undefined,
...
export const weatherForecastTool: Tool<WeatherForecastInput, WeatherForecastData | { locations: WeatherForecastData[] }> = {
  name: "weather_forecast",
  description: "Get local or South Korea nationwide weather forecasts for today, tomorrow, weekend, or this week using Open-Meteo.",
  ...
  async run(input, context) {
    ...
    if (scope === "korea_nationwide") {
      const summaries = await Promise.all(koreaForecastLocations.map(async (location) => {
...
function queryTerms(query: string) {
  return query
    ...
    .split(/[\s,./|+_-]+/)
    .map((term) => term.trim())
    .filter((term) => term.length >= 2 && !["the", "and", "for", "with", "news", "latest", "오늘", "최신"].includes(term));
}
...
function normalizeYahooMarketSymbol(symbol: string, market: string | null | undefined) {
  ...
  const clean = raw.toLowerCase().replace(/[^a-z0-9.^\s가-힣-]/g, "").replace(/\s+/g, " ").trim();
...
export const marketQuoteTool: Tool<MarketInput, MarketQuote> = {
  name: "market_quote",
  description: "Get a current stock, index, fund, market status, 주식, 증시, 코스피, 코스닥, KOSPI, or KOSDAQ quote using Stooq CSV data. Use this before news_latest or web_search for stock-market status questions.",
...
function parseSportsEvents(payload: unknown, team: string | null | undefined, maxEvents: number): SportsEvent[] {
  ...
  return events.map((event): SportsEvent | undefined => {
    if (typeof event !== "object" || event === null) return undefined;
    const record = event as Record<string, unknown>;
    const competitions = Array.isArray(record.competitions) ? record.competitions : [];
    const competition = competitions.find((item) => typeof item === "object" && item !== null) as Record<string, unknown> | undefined;
    ...
    const parsedCompetitors = competitors.flatMap((competitor) => {
      ...
      const teamRecord = typeof competitorRecord.team === "object" && competitorRecord.team !== null
    ...
    if (teamKey && !parsedCompetitors.some((competitor) => {
      return normalizeTeam(`${competitor.name} ${competitor.abbreviation ?? ""}`).includes(teamKey);
    })) {
      return undefined;
    }

    const status = typeof record.status === "object" && record.status !== null
    ...
    return {
      ...
      status: typeof status?.shortDetail === "string"
        ? status.shortDetail
  ...
  }).filter((event): event is SportsEvent => event !== undefined).slice(0, maxEvents);
}
