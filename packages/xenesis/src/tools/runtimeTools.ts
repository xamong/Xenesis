...
function hasFailureLikeDiagnosticOutput(stdout: string, stderr: string) {
  ...
  return output.split(/\r?\n/).some((line) => {
    ...
    const withoutZeroFailureCounters = trimmed.replace(
      /\b(?:failed|failures?|errors?|error|exceptions?)\s*[:=]\s*0\b/ig,
      ""
    );
...
function looksLikeGenericServerLandingPage(text: string) {
  return /IIS Windows Server|HTTP Error \d|HTTP 오류 \d|Apache(?:2)? (?:Debian )?Default Page|nginx welcome|It works!/i.test(text);
}
...
async function buildServerStartInvocation(options: {
  command: string;
  cwd: string;
  readinessUrl: string;
}) {
  ...
  async run(input, context) {
    if (input.action === "list") {
      const lines = Array.from(managedServers.values()).map((server) => {
        const status = server.child.exitCode === null && !server.child.killed ? "running" : `exited(${server.child.exitCode ?? "unknown"})`;
