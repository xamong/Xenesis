...
import { type RipgrepExecutable, ripgrepExecutables } from './ripgrep.js';
...
async function collectSearchFiles(workspaceRoot: string, current: string, files: string[]): Promise<void> {
...
function runRipgrep(
  ...
  args: string[],
  cwd: string,
): Promise<{ type: 'result'; result: { ok: boolean; content: string } } | { type: 'unavailable' }> {
  return new Promise((resolve) => {
    ...
    child.on('error', (error) => {
      if (/ENOENT/i.test(error.message)) finish({ type: 'unavailable' });
        ...
        finish({
          type: 'result',
          result: { ok: false, content: `Failed to run ${executable.source} rg: ${error.message}` },
        });
    });
    child.on('close', (code) => {
      if (settled) return;
      if (code === 0) finish({ type: 'result', result: { ok: true, content: truncateSearchOutput(stdout.trimEnd()) } });
        ...
        finish({
          type: 'result',
          result: { ok: false, content: stderr.trimEnd() || `${executable.source} rg exited with code ${code}` },
        });
    });
  });
}
