...
function taskPrompt(title: string, task: HandoffTaskItem, index: number, total: number, plan: string | null | undefined) {
  ...
  async run(input, context) {
    try {
      ...
      if (action === "status") {
        ...
        if (summaries.length === 0) return { ok: false, content: `Task handoff failed: handoff not found: ${handoffId}` };
      ...
      for (const item of prepared) {
        ...
        queued.push(await store.create({
          id: item.id,
