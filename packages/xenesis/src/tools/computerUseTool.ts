...
interface CaptureElementPayload { role?: unknown; label?: unknown }
    ...
    async run(input, context) {
      ...
      try {
        ...
        if (action === "list_apps") {
          ...
          const result = isRecord(payload.result) ? payload.result : {};
          const rawApps = Array.isArray(result.apps) ? result.apps : (Array.isArray(payload.result) ? payload.result : []);
          const lines = rawApps.map((app: unknown) => {
        ...
        return { ok: true, content: `computer_use ${action}: ${String(payload.path ?? `xd.computer.${action}`)}`, data: payload };
      } catch (error) {
