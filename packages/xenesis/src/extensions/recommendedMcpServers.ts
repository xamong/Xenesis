...
function resolveTemplateString(value: string, options: { workspaceRoot: string; env: Record<string, string | undefined> }) {
...
function resolveTemplateEnv(env: Record<string, string> | undefined, options: { workspaceRoot: string; env: Record<string, string | undefined> }) {
...
export function resolveRecommendedServer(
  ...
  const base: McpServerConfig = server.template.type === "stdio"
    ? {
      type: "stdio",
