...
export function maxMemorySensitivity(...values: Array<MemorySensitivity | undefined>): MemorySensitivity {
  return values.reduce<MemorySensitivity>((current, next) => {
    const candidate = next ?? 'low';
    return SENSITIVITY_RANK[candidate] > SENSITIVITY_RANK[current] ? candidate : current;
  }, 'low');
}

export function effectiveMemoryInputSensitivity(input: MemoryInput, declared?: MemorySensitivity): MemorySensitivity {
...
export function classifyMemoryEvidenceSensitivity(evidence: MemoryEvidenceRecord): MemorySensitivity {
  ...
  return effectiveMemoryInputSensitivity(
    {
      id: evidence.id,
      text: [evidence.source, evidence.summary ?? '', evidence.uri ?? '', metadata].join(' '),
      tags: [],
    },
