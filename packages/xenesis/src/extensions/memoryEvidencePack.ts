...
function confidenceFor(input: {
  ...
  const hasEvidence = input.records.some((record) =>
    (record.evidenceIds ?? []).some((id) => input.visibleEvidenceIds.has(id)),
  );
...
function sortByAuthority(records: MemoryRecord[], at: string, visibleEvidenceIds: ReadonlySet<string>): MemoryRecord[] {
  return [...records].sort((left, right) => {
    ...
    return (
      Date.parse(right.validFrom ?? right.updatedAt) - Date.parse(left.validFrom ?? left.updatedAt) ||
...
export function buildMemoryEvidencePackSystemMessage(pack: MemoryEvidencePack): SystemMessage | undefined {
  ...
  const proposalLines = pack.proposals.map(
    (proposal) =>
