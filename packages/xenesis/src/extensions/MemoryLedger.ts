...
export class MemoryLedger {
  ...
  async createProposal(
    ...
    const decision =
      decisionOverride ??
      classifyMemoryWrite(normalizedInput, { ...proposalContext, intent: proposalContext.intent ?? 'propose' });
  ...
  async recordEvidence(
    input: Omit<MemoryEvidenceRecord, 'createdAt'> &
      Partial<Pick<MemoryEvidenceRecord, 'createdAt' | 'updatedAt' | 'status'>>,
    context: MemoryWriteContext,
  ): Promise<MemoryEvidenceRecord> {
  ...
  async write(input: MemoryInput, context: MemoryWriteContext): Promise<MemoryWriteResult> {
    ...
    const conflictIds = unique([
      ...(normalizedInput.conflictsWith ?? []),
      ...conflicts.map((conflict) => conflict.existingId),
    ]);
    const inputWithConflicts =
      conflictIds.length > 0 ? { ...normalizedInput, conflictsWith: conflictIds } : normalizedInput;
  ...
  async partiallySupersedeRecord(
    ...
    const base = await this.options.memoryStore.get(baseId);
    if (!base) throw new Error(`Memory record not found: ${baseId}`);
    if ((base.status ?? 'active') === 'archived')
      throw new Error(`Cannot partially supersede archived memory: ${baseId}`);
    if (base.supersededBy) throw new Error(`Memory record is already superseded: ${baseId}`);
    ...
    try {
      ...
      const acceptedProposal = await this.options.ledgerStore.updateProposal(proposal.id, (current) => {
        if (current.status !== 'pending')
          throw new Error(`Memory proposal is already ${current.status}: ${proposal.id}`);
        return {
          ...current,
          status: 'accepted',
          decision,
  ...
  private async resolveMemoryEvidenceAttachment(
    ...
    const evidenceIds = unique(
      [...(input.evidenceIds ?? []), ...(context.evidenceIds ?? [])].filter((id) => id.trim()),
    );
    ...
    return {
      noEvidenceReason: input.noEvidenceReason?.trim() || `trusted ${context.sourceKind} without durable evidence`,
    };
  }
