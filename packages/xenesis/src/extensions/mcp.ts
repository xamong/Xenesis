...
import type { McpHttpServerConfig, McpServerConfig, McpStdioServerConfig } from '../config/index.js';
 ...
 */
export function buildMcpStdioEnv(configEnv: Record<string, string> | undefined): Record<string, string> | undefined {
...
const nonstandardInvalidGrantAliases = new Set(['invalid_refresh_token', 'expired_refresh_token', 'token_expired']);
...
function normalizedOAuthError(value: unknown): Record<string, unknown> | undefined {
  ...
  return Object.fromEntries(
    Object.entries(value).filter(
      ([key, nextValue]) => ['error', 'error_description', 'error_uri'].includes(key) && typeof nextValue === 'string',
    ),
  );
}
...
function scopeIncludesAll(currentScope: string | undefined, requestedScope: string) {
  ...
  return requestedScope
    .split(/\s+/u)
    .filter(Boolean)
    .every((scope) => currentScopes.has(scope));
}
...
export function evaluateMcpTokenState(
  ...
  const needsStepUp =
    options.pendingStepUpScope !== undefined && !scopeIncludesAll(entry.scope, options.pendingStepUpScope);
  if (needsStepUp) {
    return {
  ...
  return {
    ...
    discoveryState(): OAuthDiscoveryState | undefined {
      ...
      return {
        ...
        ...(entry.discoveryState.resourceMetadataUrl
          ? { resourceMetadataUrl: entry.discoveryState.resourceMetadataUrl }
          : {}),
      };
    },
...
export async function runMcpOAuthLogin(options: RunMcpOAuthLoginOptions): Promise<'authorized' | 'redirect'> {
  ...
  const clientSecret =
    oauth.clientSecret !== undefined ? await resolveSecretRef(oauth.clientSecret, options.env) : undefined;
...
function resourceSummary(resources: McpResourceDefinition[]) {
  return resources.map((resource) => (resource.name ? `${resource.name} (${resource.uri})` : resource.uri)).join(', ');
}
...
async function listMcpResources(
  ...
  const entries = Object.entries(clients).filter(([serverName]) => !targetServer || serverName === targetServer);
...
export async function createMcpTools(options: CreateMcpToolsOptions): Promise<Tool[]> {
  ...
  const tools: Tool[] = definitions
    .filter((definition) => matchesMcpToolFilter(definition.name, options.toolFilter))
    .map((definition) => {
      const converted = (() => {
        try {
          return jsonSchemaToZod(definition.inputSchema);
        } catch {
...
export function createMcpResourceTools(clients: Record<string, McpToolClient>): Tool[] {
  const listTool: Tool<
    ...
    Array<McpResourceDefinition & { server: string }>
  > = {
...
export interface CreateXenesisMcpServerOptions {
  ...
  context:
    | ToolContext
    | ((request: { toolName: string; arguments: Record<string, unknown> }) => ToolContext | Promise<ToolContext>);
...
async function callXenesisToolForMcp(
  ...
  if (!parsedInput.success) {
    throw new Error(
      `Tool ${toolName} input is invalid: ${parsedInput.error.issues.map((issue) => issue.message).join('; ')}`,
    );
  }
...
export function createXenesisMcpServer(options: CreateXenesisMcpServerOptions): Server {
  ...
  const server = new Server(options.serverInfo ?? { name: 'xenesis', version: '0.1.0' }, {
    capabilities: { tools: {} },
  });
    ...
    if (!this.breaker.canAttempt()) {
      return {
        ...
        content: [
          {
            type: 'text',
            text: `MCP server "${this.serverName}" temporarily disabled (circuit open after repeated failures).`,
          },
        ],
      } as McpToolResult;
    }
...
export class RemoteMcpToolClient implements McpToolClient {
  ...
  private async createAuthProvider(): Promise<OAuthClientProvider | undefined> {
    ...
    const clientSecret = oauth.clientSecret !== undefined ? await resolveSecretRef(oauth.clientSecret) : undefined;
