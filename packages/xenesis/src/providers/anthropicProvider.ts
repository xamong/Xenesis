...
function createRequest(request: ProviderRequest, defaultModel: string, maxTokens: number, providerName: string, stream?: boolean) {
...
export class AnthropicProvider implements AgentProvider {
  ...
  constructor(options: AnthropicProviderOptions) {
    ...
    this.client = options.client ?? new FetchAnthropicMessagesClient({
  ...
  async *stream(request: ProviderRequest): AsyncIterable<ProviderStreamEvent> {
    ...
    for await (const event of responseStream) {
      ...
      if (record.type === "error") {
        ...
        throw new Error(`${this.name} provider stream failed for model "${createPayload.model}": ${String(error.message ?? "unknown error")}`);
      }
    }
