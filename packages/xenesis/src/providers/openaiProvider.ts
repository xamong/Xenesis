...
function normalizeOpenAISchema(value: unknown, key?: string): unknown {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([entryKey, entryValue]) => [
      entryKey,
      normalizeOpenAISchema(entryValue, entryKey)
    ])
  );
}
...
function responseWithStreamedOutputFallback(
  response: unknown,
...
export class OpenAIProvider implements AgentProvider {
  ...
  async complete(request: ProviderRequest): Promise<ProviderResponse> {
    ...
    const createRequest: ResponseCreateParamsNonStreaming = {
      model,
      input: toOpenAIInput(request.messages, { supportsVision: supportsVision(model, this.name) }) as ResponseCreateParamsNonStreaming["input"],
    ...
    try {
      response = await this.client.responses.create(createRequest, request.signal ? { signal: request.signal } : undefined);
    } catch (error) {
  ...
  async *stream(request: ProviderRequest): AsyncIterable<ProviderStreamEvent> {
    ...
    const createRequest: ResponseCreateParamsStreaming = {
      model,
      input: toOpenAIInput(request.messages, { supportsVision: supportsVision(model, this.name) }) as ResponseCreateParamsStreaming["input"],
    ...
    try {
      responseStream = await this.client.responses.create(createRequest, request.signal ? { signal: request.signal } : undefined);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
    ...
    for await (const event of responseStream) {
      ...
      if (record.type === "response.function_call_arguments.delta" && typeof record.delta === "string") {
        appendStreamedFunctionArguments(
          functionCalls,
          functionCallOrder,
          streamItemKey(record),
          record.delta
        );
      }
    ...
    if (completedResponse === undefined) {
      throw new Error(`${providerLabel(this.name)} provider stream ended without a completed response for model "${model}".`);
    }

    yield {
      type: "response",
      response: providerResponse(responseWithStreamedOutputFallback(
        completedResponse,
