...
function lastToolCall(messages: AgentMessage[], name: string) {
  return assistantToolCalls(messages).reverse().find((toolCall) => toolCall.name === name);
}

function toolResultForCall(messages: AgentMessage[], toolCall: ToolCall) {
  return [...messages].reverse().find((message): message is Extract<AgentMessage, { role: "tool" }> => (
    message.role === "tool" && message.toolCallId === toolCall.id
  ));
}

function lastDeniedWriteCall(messages: AgentMessage[]) {
  return assistantToolCalls(messages).reverse().find((toolCall) => {
    if (toolCall.name !== "write") return false;
    const result = toolResultForCall(messages, toolCall);
    return result ? mockVisibleContent(result.content).includes("requires prior evidence tool before file mutation") : false;
  });
}
...
function hasSelfRunAppReadinessScenario(messages: AgentMessage[]) {
  return messages.some((message) => message.role === "user" && message.content.startsWith(selfRunAppReadinessPrefix)) ||
...
function latestSystemMessage(messages: AgentMessage[]) {
  return [...messages].reverse().find((message): message is Extract<AgentMessage, { role: "system" }> => message.role === "system");
}
...
function selfRunAppReadinessResponse(request: ProviderRequest): ProviderResponse | undefined {
  ...
  if (
    last?.role === "system" &&
    latestSystem.includes("Run the next app verification step: app_readiness")
  ) {
  ...
  if (last?.role === "tool" && last.name === "app_readiness") {
    ...
    return {
      message: { role: "assistant", content: "Readiness failed; I need to inspect and repair the concrete client defect." }
    };
  }

  if (
    last?.role === "system" &&
    latestSystem.includes("Run the next app verification step: diagnostics")
  ) {
  ...
  if (
    last?.role === "system" &&
    latestSystem.includes("Run the next app verification step: app_e2e_check")
  ) {
...
export class MockProvider implements AgentProvider {
  ...
  async complete(request: ProviderRequest): Promise<ProviderResponse> {
    ...
    if (last?.role === "tool") {
      ...
      const deniedWrite = last.name === "write" && lastToolContent.includes("requires prior evidence tool before file mutation")
