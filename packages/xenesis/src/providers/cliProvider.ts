...
const DESK_MCP_TOOL_NAMES = ['xenesis_desk_capabilities', 'xenesis_desk_capability', 'xenesis_desk_call_capability'];
...
function codexDeskMcpArgs(env: NodeJS.ProcessEnv) {
  ...
  const args = [
    ...
    '-c',
    `mcp_servers.${serverKey}.env.XENIS_MCP_STATE_FILE=${tomlString(config.stateFilePath)}`,
    '-c',
