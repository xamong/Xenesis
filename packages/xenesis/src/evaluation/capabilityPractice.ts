...
import { z } from 'zod';
import { type ApprovalMode, loadConfig, type ProviderName } from '../config/index.js';
...
function resolveScenarioWorkspace(baseWorkspace: string, scenario: CapabilityPracticeScenario) {
  ...
  return isAbsolute(scenario.workspace) ? resolve(scenario.workspace) : resolve(baseWorkspace, scenario.workspace);
}
...
async function defaultPracticeRunner(input: CapabilityPracticeRunnerInput): Promise<CapabilityPracticeRunnerResult> {
...
export async function runCapabilityPracticeSuite(
  ...
  const scenarios = (options.scenarios ?? defaultCapabilityPracticeScenarios).slice(
    0,
    options.maxScenarios ?? Number.MAX_SAFE_INTEGER,
  );
...
export async function readCapabilityPracticeScenarioFile(path: string) {
  ...
  return parsed.map((item, index) => {
    try {
      ...
      throw new Error(
        `Invalid practice scenario ${index} in ${path}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  });
}
