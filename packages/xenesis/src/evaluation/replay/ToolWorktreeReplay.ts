...
export async function collectToolWorktreeObservation(options: ToolWorktreeReplayOptions): Promise<OracleObservation> {
  ...
  return {
    ...
    visibleResult:
      'worktree tools validate names, create durable git worktree sessions, block duplicate entry, keep or remove worktrees safely, and require explicit discard confirmation for dirty removal',
  };
}
