...
export async function collectToolAgentTaskObservation(options: ToolAgentTaskReplayOptions): Promise<OracleObservation> {
  ...
  return {
    ...
    visibleResult:
      'agent launch and agent task lifecycle tools create durable tasks, expose status, update dependencies and metadata, stop running tasks, and render completed task output',
  };
}
