import { createMcpResourceTools, type McpReadResourceResult, type McpToolClient } from '../../extensions/mcp.js';
...
export async function collectToolMcpResourcesObservation(
  ...
  const readReadonly = await allRead.run(
    { server: names.readonly.name, uri: names.readonly.resources?.[0]?.uri ?? '' },
    toolContext,
  );
  ...
  return {
    ...
    visibleResult:
      'MCP resource tools list server-filtered resources, isolate list failures, read text resources, and report missing or unreadable servers deterministically',
  };
}
