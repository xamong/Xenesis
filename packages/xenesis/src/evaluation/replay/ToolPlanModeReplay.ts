...
export async function collectToolPlanModeObservation(options: ToolPlanModeReplayOptions): Promise<OracleObservation> {
  ...
  return {
    ...
    visibleResult:
      'plan mode tools persist read-only planning state, reject exit before entry, return approved saved or inline plans, preserve allowed prompts, and restore work mode',
  };
}
