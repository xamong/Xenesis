...
export async function collectLegacySessionMigrationObservation(xenesisHome: string): Promise<OracleObservation> {
  ...
  return {
    ...
    visibleResult:
      'legacy channel_sessions.json string and object values migrate into SQLite channel_sessions, invalid entries are ignored, and the legacy file is marked migrated',
  };
}
