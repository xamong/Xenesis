import {
  buildFeatureParityMatrixFromSourceCatalog,
  summarizeFeatureParityMatrix
} from "./FeatureParityMatrix.js";
...
export function createParityReportPayload(catalog: SourceFeatureCatalog) {
  ...
  return {
    ...
    featureParity: {
      ...
      implemented: featureParityMatrix.items
        .filter((item) => item.status === "implemented")
        .map(parityItemSummary),
      notStarted: featureParityMatrix.items
