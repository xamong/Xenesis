...
export function rankIdeContextFiles(context: IdeContext): RankedIdeFile[] {
  ...
  return Array.from(ranks.values()).sort(
...
export function buildIdeContextSystemMessage(
  context: IdeContext,
): Extract<AgentMessage, { role: 'system' }> | undefined {
  ...
  if (rankedFiles.length > 0) {
    ...
    for (const file of rankedFiles) {
      lines.push(
        `<related_file path="${xmlEscape(file.path)}" score="${file.score}" reasons="${xmlEscape(file.reasons.join(','))}" />`,
      );
    }
    lines.push('</related_files>');
  }
