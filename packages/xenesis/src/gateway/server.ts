...
import { loadPluginWorkflows, type PluginWorkflowDescriptor, SqlitePluginStateStore } from '../extensions/index.js';
import {
  type AgentTask,
...
function isObservabilityEventKind(value: unknown): value is GatewayObservabilityEventKind {
  return value === 'request' || value === 'response' || value === 'retry' || value === 'error' || value === 'task';
}
  ...
  if (status !== null) {
    if (!isAgentTaskStatus(status)) {
      throw new GatewayHttpError(
        400,
        'Query "status" must be queued, running, completed, failed, cancelled, or blocked.',
      );
    }
    query.status = status;
  }
...
function filterObservabilityEvents(events: GatewayObservabilityEvent[], query: GatewayObservabilityQuery = {}) {
  let filtered = events;
...
function gatewayArgv(options: GatewayOptions, body: GatewayWorkflowBody) {
  const argv = ['node', 'xenesis', '--cwd', options.cwd, '--json', '--print'];
...
function gatewayActionArgv(options: GatewayOptions, args: string[]) {
  const argv = ['node', 'xenesis', '--cwd', options.cwd];
...
function gatewayCliOverrides(options: GatewayOptions): CliConfigOverrides | undefined {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--home') {
      ...
      parsed.providerFallbacks = [...(parsed.providerFallbacks ?? []), { provider: provider! }];
      index += 1;
...
async function listSessions(options: GatewayOptions) {
  ...
  return sessions.sort((left, right) => right.mtimeMs - left.mtimeMs).map((session) => session.id);
}
...
async function listTraceSessionData(options: GatewayOptions, traceId: string) {
  return (await listAllTraceSessionData(options)).filter((session) => session.traceId === traceId);
}
...
async function listTasks(options: GatewayOptions, query: GatewayTaskQuery = {}) {
  ...
  return query.limit !== undefined && sorted.length > query.limit ? sorted.slice(0, query.limit) : sorted;
}
...
async function listSchedules(options: GatewayOptions) {
  return (await (await gatewayScheduleStore(options)).list()).sort((left, right) =>
...
function reportMatchesStatus(report: { exitCode: number; failed: number }, status: GatewayReportStatus | undefined) {
    ...
    ...(report.verification
      ? {
          verification: {
            ...
            failedCommands: report.verification.results.filter((result) => !result.ok).map((result) => result.command),
          },
        }
      : {}),
  };
...
async function listRunReportsByTraceId(options: GatewayOptions, traceId: string) {
  return (await listTraceRunReports(options)).filter((report) => report.traceId === traceId);
}
...
function traceSessionIds(
  ...
  return new Set(
    [...sessions.map((session) => session.status.id), ...reports.map((report) => report.sessionId)].filter(
...
function summarizeTaskExecution(tasks: AgentTask[]) {
  const handoffIds = Array.from(
    new Set(tasks.map((task) => task.handoffId).filter((handoffId): handoffId is string => Boolean(handoffId?.trim()))),
  ).sort();
...
function buildTraceDiagnostics(
  ...
  const failedToolCalls = records
    .filter(
      (record): record is SessionLogRecord & { type: 'tool_result'; ok: false } =>
  ...
  const permissionIssues = records
    .filter(
      (record): record is SessionLogRecord & { type: 'permission_audit' } =>
  ...
  const toolPolicyIssues = records
    .filter(
      (record): record is SessionLogRecord & { type: 'tool_policy_audit' } =>
  ...
  const verification =
    verificationReports.length > 0
      ? {
          status:
            failedCommands.length > 0 || verificationReports.some((report) => report.status === 'failed')
              ? 'failed'
  ...
  const handoffDependencyCount = runReports.reduce(
    (sum, report) => sum + (report.metrics?.handoffDependencyCount ?? 0),
    0,
  );
  const toolRecoveryItems = runReports.flatMap((report) =>
    (report.toolRecovery?.items ?? []).map((item) => ({
      sessionId: report.sessionId,
  ...
  const toolRecoveryHintCount = runReports.reduce(
    (sum, report) => sum + (report.metrics?.toolRecoveryHintCount ?? report.toolRecovery?.total ?? 0),
    0,
  );
  const toolRecoveryRecoveredCount = runReports.reduce(
    (sum, report) => sum + (report.metrics?.toolRecoveryRecoveredCount ?? report.toolRecovery?.recoveredCount ?? 0),
    0,
  );
  const toolRecoveryUnrecoveredCount = runReports.reduce(
    (sum, report) => sum + (report.metrics?.toolRecoveryUnrecoveredCount ?? report.toolRecovery?.unrecoveredCount ?? 0),
    0,
  );
...
function summarizeDecisionTraceStatus(reports: RunReport[]) {
  const latest = reports
    .flatMap((report) =>
      (report.decisionTrace ?? []).map((decision) => ({
        sessionId: report.sessionId,
        ...(report.traceId ? { traceId: report.traceId } : {}),
        reportId: report.id,
        createdAt: report.createdAt,
...
function summarizeTopFailedTools(reports: RunReport[]) {
  ...
  return Array.from(summaries.values())
    .sort(
      (left, right) =>
        right.failures - left.failures || right.runCount - left.runCount || left.name.localeCompare(right.name),
    )
    .slice(0, 10);
}
...
function summarizeRepairStopReasons(reports: RunReport[]) {
  ...
  for (const report of reports) {
    for (const decision of report.decisionTrace ?? []) {
      ...
      summary.count += 1;
      summary.failedCommands = uniqueInOrder([...summary.failedCommands, ...(decision.related ?? [])]);
      summary.latestSessionId ??= report.sessionId;
      if (report.traceId) summary.latestTraceId ??= report.traceId;
...
function summarizeDiagnosticInsights(reports: RunReport[], tasks: Awaited<ReturnType<typeof listTasks>>) {
...
function summarizeGatewayWorkflowStatus(workflows: GatewayWorkflowSummary[]) {
  const defaultWorkflow =
    workflows.find(
      (workflow) => (workflow.metadata as { defaultWorkflow?: unknown } | undefined)?.defaultWorkflow === true,
...
function recordGatewayChannelRuntimeError(
  ...
  const shouldRecord =
    !previous || previous.message !== message || now - (health[channel]?.lastRecordedErrorAt ?? 0) > 60_000;
...
async function createGatewayChannelRuntime(
  ...
  if (slack?.enabled) {
    ...
    managers.push(
      createManagedChannel(options, config, slackAdapter, {
  ...
  return createServer(async (request, response) => {
    ...
    try {
      ...
      if (url.pathname === '/channels/slack/interactions') {
        ...
        const result = await channels.slack.handleInteractionRequest(
          request.headers,
      ...
      if (request.method === 'POST' && url.pathname === '/observability/events') {
        const acceptedEvents = await observabilityStore.record(
          parseObservabilityBody(await readJsonBody(request, bodyLimit)),
        );
        writeJson(response, 200, {
      ...
      if (request.method === 'POST' && url.pathname === '/checks/connect') {
        ...
        writeJson(
          response,
          200,
          await runCliAction(options, ['--json', 'connect', 'check', ...(probe ? ['--probe'] : [])]),
        );
        return;
      }
      ...
      if (request.method === 'POST' && taskActionMatch) {
        ...
        writeJson(
          response,
          200,
          await runCliAction(options, action === 'run' ? ['tasks', 'run', id, '--print'] : ['tasks', 'cancel', id]),
        );
        return;
      }
...
export async function startGateway(options: GatewayOptions): Promise<GatewayHandle> {
  ...
  const channelManagers: ChannelManager[] = channelRuntime.managers;
