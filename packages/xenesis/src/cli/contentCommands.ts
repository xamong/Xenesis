...
import { compactSessionEvents, eventsToMessages, readSessionLog, type SessionLogRecord } from '../sessions/index.js';
...
function deriveSessionName(records: SessionLogRecord[]) {
  const text =
    eventsToMessages(records).find((message) => message.role === 'user' && message.content.trim().length > 0)
...
export async function renderExportCommand(config: XenesisConfig, requestedSessionId?: string, filename?: string) {
  const { sessionId, records } = await readResolvedSession(config, requestedSessionId);
...
export async function renderSummaryCommand(config: XenesisConfig, requestedSessionId?: string) {
  const { sessionId, records } = await readResolvedSession(config, requestedSessionId);
  return [`summary: session=${sessionId}`, 'summary: providerCalls=false', compactSessionEvents(records)];
}
