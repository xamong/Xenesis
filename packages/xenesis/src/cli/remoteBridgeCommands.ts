...
const commandSpecs: Record<RemoteBridgeCommandName, RemoteBridgeCommandSpec> = {
  bridge: {
    ...
    referenceBehavior: "Checks remote-control policy, bridge version, access token, and toggles bidirectional bridge AppState.",
    localBehavior: "Prints bridge setup intent and local-only diagnostics without registering environments or opening sockets.",
    setupIntent: "Review whether a remote-control bridge should be configured, then use an explicit integration outside this bounded CLI if live control is required.",
    prompt: "Draft a local checklist for remote-control bridge readiness, including policy, token, version, and teardown considerations, without connecting to any service.",
    forbidden: [
      ...sharedForbidden,
  ...
  "bridge-kick": {
    ...
    referenceBehavior: "Injects bridge debug fault states into a live Remote Control session for manual recovery testing.",
    localBehavior: "Echoes the requested recovery scenario as a dry-run diagnostic and never mutates a bridge debug handle.",
    setupIntent: "Capture a bridge recovery test scenario as local notes before running any live recovery exercise elsewhere.",
    prompt: "Turn the requested bridge-kick scenario into a local recovery test plan with expected observations and rollback steps, without injecting faults.",
    forbidden: [
      ...sharedForbidden,
  ...
  chrome: {
    ...
    referenceBehavior: "Inspects Chrome extension state, opens extension URLs, toggles default config, and reports MCP connection state.",
    localBehavior: "Prints browser integration setup intent without launching Chrome, probing extensions, or editing browser permissions.",
    setupIntent: "Document desired Chrome integration setup and permission posture before the user configures a browser extension manually.",
    prompt: "Build a local Chrome integration readiness checklist covering extension installation, reconnect, permissions, and default enablement without opening a browser.",
    forbidden: [
      ...sharedForbidden,
  ...
  desktop: {
    ...
    setupIntent: "Record the desired desktop handoff context so the user can perform any live desktop setup explicitly.",
    prompt: "Draft a local desktop handoff note that names the workspace, expected desktop surface, and validation steps without launching an app.",
    forbidden: [
      ...sharedForbidden,
  ...
  mobile: {
    ...
    referenceBehavior: "Shows a QR-code flow for downloading or opening the Claude mobile app; aliases include ios and android.",
    ...
    prompt: "Prepare a local mobile handoff checklist covering target platform, QR/link needs, and privacy expectations without contacting app stores.",
    forbidden: [
      ...sharedForbidden,
  ...
  teleport: {
    ...
    localBehavior: "Reports teleport as local diagnostic intent only and does not emulate hidden remote session behavior.",
    setupIntent: "Capture teleport-related requirements as local notes; no remote session is created.",
    prompt: "Summarize what a teleport-like remote session would need, including environment, trust, and teardown boundaries, without creating it.",
    forbidden: [
      ...sharedForbidden,
      "No teleport session, tunnel, remote shell, or hidden command path is activated."
    ]
  },
  "remote-env": {
    command: "remote-env",
    ...
    referenceBehavior: "Gates a local JSX flow by subscription and policy to configure the default remote environment for teleport sessions.",
    localBehavior: "Prints remote environment setup intent without checking subscription policy or saving remote defaults.",
    setupIntent: "Record desired remote environment defaults locally as planning context only.",
    prompt: "Draft a local remote environment requirements checklist covering language runtimes, cwd, env vars, and network policy without provider calls.",
    forbidden: [
      ...sharedForbidden,
  ...
  "remote-setup": {
    ...
    referenceBehavior: "Imports GitHub tokens, checks OAuth credentials, fetches environments, and creates cloud defaults through remote APIs.",
    localBehavior: "Prints remote setup intent without reading GitHub tokens, starting OAuth, fetching environments, or creating cloud defaults.",
    setupIntent: "Document remote setup prerequisites and token-handling expectations before the user performs explicit remote setup.",
    prompt: "Create a local remote setup plan covering sign-in, GitHub token handling, default environment shape, and failure handling without calling APIs.",
    forbidden: [
      ...sharedForbidden,
  ...
  "terminal-setup": {
    ...
    referenceBehavior: "Opens a terminal-specific setup flow for Shift+Enter or Option+Enter behavior unless the terminal natively supports CSI u.",
    ...
    prompt: "Draft a local terminal setup checklist for multiline input keybindings and visual-bell preferences without editing terminal settings.",
    forbidden: [
      ...sharedForbidden,
  ...
  ide: {
    ...
    referenceBehavior: "Detects IDEs, can install extensions, opens projects, and mutates dynamic IDE MCP configuration.",
    localBehavior: "Prints IDE integration setup intent without process detection, extension installation, project opening, or MCP connection.",
    setupIntent: "Record IDE integration requirements so the user can configure an extension or MCP bridge explicitly.",
    prompt: "Build a local IDE integration checklist covering target IDE, workspace match, extension/plugin status, and MCP trust without detecting IDEs.",
    forbidden: [
      ...sharedForbidden,
  ...
  voice: {
    ...
    setupIntent: "Record desired voice-mode expectations and privacy requirements before enabling any audio integration elsewhere.",
    prompt: "Draft a local voice-mode readiness checklist covering microphone consent, transcription boundaries, and disable behavior without starting audio.",
    forbidden: [
      ...sharedForbidden,
      "No microphone, audio stream, voice service, or feature-gate check is activated."
    ]
  }
};
...
function buildPayload(input: RemoteBridgeCommandInput) {
  ...
  return {
    ...
    prompt: requestedArgs.length > 0
      ? `${spec.prompt}\n\nUser-supplied context: ${requestedArgs.join(" ")}`
      : spec.prompt,
