...
import { loadConfig, type CliConfigOverrides } from "../config/index.js";
...
export async function runDoctor(options: DoctorOptions = {}): Promise<DoctorResult> {
  ...
  try {
    ...
    lines.push(`providerFallbacks: ${config.providerFallbacks.map((fallback) => fallback.provider).join(", ") || "none"}`);
    lines.push(`context: autoCompact=${config.context.autoCompact}, compactAfter=${config.context.compactAfterMessages}, keep=${config.context.compactKeepMessages}, maxToolResultChars=${config.context.maxToolResultChars}`);
    if (config.browser.enabled) {
      try {
        await import("playwright");
        lines.push("browser: playwright ok");
      } catch {
        lines.push("browser: error (playwright not installed; run `npm install playwright && npx playwright install chromium`)");
        exitCode = 1;
      }
    } else {
    ...
    if (config.provider !== "mock" && providerSettings.apiKeyEnv) {
      lines.push(env[providerSettings.apiKeyEnv]
