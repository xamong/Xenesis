...
export const modePromptCommandNames = ['ultraplan', 'fast', 'thinkback', 'thinkback-play'] as const;
...
function buildFastPayload(
  args: string[],
  runtime: ModePromptRuntimeIntent,
): ModePromptCommandPayload | { error: string } {
...
function buildThinkbackPlayPayload(
  args: string[],
  runtime: ModePromptRuntimeIntent,
): ModePromptCommandPayload | { error: string } {
  if (args.length > 0) {
...
export function renderModePromptCommand(options: {
  ...
  const payloadOrError =
    options.command === 'ultraplan'
