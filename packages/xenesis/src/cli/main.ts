...
import { type AgentProvider, resolveProviderSettings } from '../providers/index.js';
...
async function runMemoryCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  const records = parsed.memoryCommand === 'search' ? await store.search(parsed.memoryQuery ?? '') : await store.list();
