...
import { runAgentPipeline, resumeAgentPipeline } from "../core/AgentRunPipeline.js";
import type { ApprovalHandler } from "../core/AgentRunner.js";
import type { AgentRunEvent } from "../core/events.js";
import type { AgentMessage } from "../core/messages.js";
import {
  createProvider as createResolvedProvider,
  resolveRuntimeMcpServers,
  type AgentRunMode
} from "../core/AgentRuntimeFactory.js";
import {
...
import {
  isDiagnosticCommandName,
  renderDiagnosticCommand,
  type DiagnosticCommandName
} from "./diagnosticCommands.js";
import {
...
import {
  isPreferenceCommandName,
  runPreferenceCommand,
  type PreferenceCommandName
} from "./preferenceCommands.js";
import {
...
function normalizeArgv(argv: string[]) {
  ...
  const second = String(argv[1] ?? "").replace(/\\/g, "/").toLowerCase();
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--provider") {
      ...
      parsed.providerRetries = parseNonNegativeInteger(readOptionValue(args, index, "--provider-retries"), "--provider-retries");
      index += 1;
      ...
      parsed.gatewayMaxBodyBytes = parsePositiveInteger(readOptionValue(args, index, "--max-body-bytes"), "--max-body-bytes");
      index += 1;
    } else if (arg === "--max-runs") {
      parsed.gatewayMaxConcurrentRuns = parsePositiveInteger(readOptionValue(args, index, "--max-runs"), "--max-runs");
      index += 1;
    } else if (arg === "--request-timeout-ms") {
      parsed.gatewayRequestTimeoutMs = parsePositiveInteger(readOptionValue(args, index, "--request-timeout-ms"), "--request-timeout-ms");
      index += 1;
    } else if (arg === "--observability-max-events") {
      parsed.gatewayObservabilityMaxEvents = parsePositiveInteger(readOptionValue(args, index, "--observability-max-events"), "--observability-max-events");
      index += 1;
    } else if (arg === "--observability-max-age-days") {
      parsed.gatewayObservabilityMaxAgeDays = parsePositiveInteger(readOptionValue(args, index, "--observability-max-age-days"), "--observability-max-age-days");
      index += 1;
    } else if (arg === "--config") {
  ...
  if (positionals[0] === "reload-plugins") {
  ...
  } else if (commandName === "usage" || commandName === "cost" || commandName === "stats" || commandName === "logout" || commandName === "rate-limit-options") {
    parsed.command = commandName;
    if (positionals.length > 1) {
    ...
    if (effortCommand === "status" || effortCommand === "current" || effortCommand === "show") {
      ...
      if (positionals.length > 2) throw new Error(`Command "effort ${effortCommand}" does not accept positional arguments.`);
    ...
    if (
      ...
      throw new Error('Command "profile" requires a subcommand: list, show, save, use, clear, delete, templates, or install.');
    }
    parsed.profileCommand = profileCommand;
    if (profileCommand === "show" || profileCommand === "save" || profileCommand === "use" || profileCommand === "delete") {
    ...
    if (sessionCommand === "show" || sessionCommand === "compact" || sessionCommand === "rewind" || sessionCommand === "resume") {
    ...
    if (normalizedAction !== "status" && normalizedAction !== "on" && normalizedAction !== "off" && normalizedAction !== "toggle") {
    ...
    if (changeCommand !== "list" && changeCommand !== "show" && changeCommand !== "revert" && changeCommand !== "accept" && changeCommand !== "diff") {
      throw new Error('Command "changes" requires a subcommand: list, show, revert, accept, or diff.');
    }
    parsed.changeCommand = changeCommand;
    if (changeCommand === "show" || changeCommand === "revert" || changeCommand === "accept" || changeCommand === "diff") {
    ...
    if (checkpointCommand !== "list" && checkpointCommand !== "show" && checkpointCommand !== "revert" && checkpointCommand !== "accept" && checkpointCommand !== "diff") {
    ...
    if (checkpointCommand === "show" || checkpointCommand === "revert" || checkpointCommand === "accept" || checkpointCommand === "diff") {
    ...
    const pluginCommand = rawPluginCommand === "remove" || rawPluginCommand === "rm"
      ? "uninstall"
      : rawPluginCommand;
    if (
      pluginCommand !== "init" &&
      ...
      throw new Error('Command "plugins" requires a subcommand: init, install, list, enable, disable, uninstall, remove, rm, update, reload, or doctor.');
    }
    ...
    const permissionCommand = positionals[0] === "allowed-tools"
      ? positionals[1] ?? "list"
      : positionals[1];
    ...
    const taskCommand = positionals[0] === "bashes"
      ? positionals[1] ?? "list"
      : positionals[1];
    if (
      taskCommand !== "start" &&
...
async function runDiagnosticCliCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  emitLines(io, await renderDiagnosticCommand(
...
async function runMiscCompatibilityCliCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  emitLines(io, await renderMiscCompatibilityCommand(
    await loadEffectiveConfig(parsed, cwd, env),
    parsed.command,
...
function modelStatusPayload(config: XenesisConfig, state: CliRuntimeState): ModelStatusPayload {
  ...
  configured.push(...config.providerFallbacks.map((fallback) => ({
...
async function runModelCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  if (parsed.modelCommand === "set") {
    ...
    emitStdout(io, `model: current=${payload.current.model} provider=${payload.current.provider} source=${payload.current.source}`);
...
async function readEffortStatus(config: XenesisConfig, env: NodeJS.ProcessEnv): Promise<EffortStatus> {
  ...
  const configured = state.effort === "auto" || isEffortLevel(String(state.effort))
    ? state.effort as EffectiveEffortLevel
    : "auto";
  const configuredSource = state.effort === "auto" || isEffortLevel(String(state.effort))
    ? "state" as const
    : "default" as const;
  ...
  const warning = envOverride && configuredSource === "state" && envOverride.effective !== configured
...
async function runEffortCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  if (command === "set") {
    ...
    if (!isAcceptedEffortInput(requested)) {
      emitStderr(io, `error: invalid effort level: ${parsed.effortLevel ?? ""}. Valid options are: low, medium, high, max, auto.`);
      return 1;
    }
    const state = await readCliRuntimeState(config);
...
function createProvider(config: XenesisConfig, env: NodeJS.ProcessEnv): AgentProvider {
  return createResolvedProvider(config, env);
}

function createFallbackProviders(config: XenesisConfig, env: NodeJS.ProcessEnv): AgentProvider[] {
  return config.providerFallbacks.map((fallback) => createProvider({
    ...config,
    provider: fallback.provider,
    model: fallback.model ?? config.model,
...
export async function createCliRuntimeTools(config: XenesisConfig, env: NodeJS.ProcessEnv, runtime: RuntimeToolsOptions = {}) {
...
function createApprovalHandler(parsed: ParsedArgs, config: XenesisConfig, io: CliIo): ApprovalHandler | undefined {
  ...
  return async (request) => {
    ...
    emitStdout(io, `Approve ${request.name}: ${request.reason} risk=${request.riskLevel}; target=${request.summary} [y/N]`);
...
async function runPluginsCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  if (stateRecords.length > 0) {
    ...
    for (const record of [...stateRecords, ...configuredOnly].sort((left, right) => left.path.localeCompare(right.path))) {
...
export async function runMcpLogin(options: {
  ...
  if (!("url" in options.serverConfig) || (options.serverConfig.type !== "http" && options.serverConfig.type !== "sse")) {
...
async function runMcpCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  if (parsed.mcpCommand === "list") {
    const entries = Object.entries(resolveRuntimeMcpServers(config, env))
...
async function runPermissionsCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  if (parsed.permissionCommand === "audit") {
    ...
    const records = (await readSessionLog(config.xenesisHome, sessionId)).filter((record): record is typeof record & {
  ...
  emitStdout(io, `toolPolicies: ${policyEntries.length > 0
    ? policyEntries
      .map(([name, policy]) => `${name}=${policy.action}${policy.reason ? ` (${policy.reason})` : ""}`)
      .join(", ")
    : "none"}`);
  emitStdout(io, `pathRules: ${config.permissions.pathRules.length > 0
...
async function runProfileCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  ...
  emitStderr(io, 'error: Command "profile" requires a subcommand: list, show, save, use, clear, delete, templates, or install.');
  return 1;
}
...
async function runConnectionCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  if (parsed.connectCommand === "latest" || parsed.connectCommand === "show") {
    ...
    const entry = parsed.connectCommand === "latest"
    ...
    if (!entry) {
      emitStderr(io, parsed.connectCommand === "latest"
    ...
    else for (const line of renderConnectionReportDetails(entry, config.xenesisHome, parsed.connectCommand === "latest" ? "latest" : "summary")) emitStdout(io, line);
    return 0;
  }
  ...
  if (parsed.json) emitStdout(io, JSON.stringify({
...
async function runSmokeCommand(parsed: ParsedArgs, cwd: string, env: NodeJS.ProcessEnv, io: CliIo) {
  if (parsed.smokeCommand === "latest" || parsed.smokeCommand === "show") {
    ...
    const entry = parsed.smokeCommand === "latest"
    ...
    if (!entry) {
      emitStderr(io, parsed.smokeCommand === "latest"
        ? "error: no smoke reports found"
        : `error: smoke report not found: ${parsed.smokeReportTarget ?? ""}`);
      return 1;
    }
    if (parsed.json) emitStdout(io, JSON.stringify({ reportPath: entry.path, report: entry.report }));
    else for (const line of renderSmokeReportDetails(entry, config.xenesisHome, parsed.smokeCommand === "latest" ? "latest" : "summary")) emitStdout(io, line);
    return 0;
  }
...
function renderVerificationReport(verification: VerificationReport | undefined) {
  ...
  const lines = [
    `verification: ${verification.status} ${verification.passed}/${verification.commandCount}`
  ];
...
function renderRunReport(report: RunReport) {
  return [
    ...
    ...report.selfReview.findings.map((finding) =>
...
async function runRunsCommand(
  ...
  if (parsed.runCommand === "report") {
    const sessionId = validateSessionId(parsed.sessionId ?? "");
    const report = await createRunReportStore(config).read(sessionId) ?? await buildAndSaveRunReport(config, sessionId);
  ...
  if (parsed.runCommand === "repair") {
    ...
    for (let attempt = 1; attempt <= config.verification.maxRepairAttempts; attempt += 1) {
      ...
      emitStdout(io, `repair: reverify ${verification?.status ?? "none"} ${verification?.passed ?? 0}/${verification?.commandCount ?? 0}`);
      const rollback = verification?.status === "failed"
        ? await rollbackFailedRepairAttempt(config, repairSessionId || "unknown", io)
        : undefined;
      const acceptance = verification?.status === "passed" && exitCode === 0
      ...
      currentReport = await reportStore.save({
        ...
        repairs: [
          ...(latest.repairs ?? []),
          createRepairRecord(currentReport, repairSessionId || "unknown", exitCode, attempt, verification?.status, rollback, acceptance)
        ]
      });
...
function renderScheduleLine(schedule: TaskSchedule) {
  ...
  const trigger =
    schedule.trigger.type === "interval" ? `interval ${schedule.trigger.every}` :
      schedule.trigger.type === "daily" ? `daily ${schedule.trigger.at}` :
...
async function runPrompt(
  ...
  const mode: AgentRunMode | undefined = parsed.command === "plan" || parsed.command === "work"
    ? parsed.command
    : undefined;
  try {
    const result = await runAgentPipeline({
      ...
      approvalHandlerFactory: (config) =>
        options.approvalHandler ?? createApprovalHandler(parsed, config, io),
...
function skillSlashMetadataMessage(commandName: string, skill: { model?: string; effort?: string; allowedTools?: string[] }) {
  const lines = [
    "Xenesis slash skill invocation:",
    `command: /${commandName}`
  ];
...
async function runChatSlashCommand(
  ...
  if (command.name === "plan" || command.name === "work") {
    ...
    await runPrompt({
      ...
      command: command.name,
      prompt
    }, cwd, env, io, prompt, setSessionWriter);
    return "continue" as const;
  }
...
async function runTuiCommand(
  ...
  const setTuiSessionWriter: SessionWriterSetter = (writer, sessionId) => {
    ...
    state = setTuiSessionContext(setTuiSuggestionContext(state, {
  ...
  const tuiApprovalHandler: ApprovalHandler = (request) => new Promise((resolve) => {
  ...
  const runTuiAgentResume = async (
    ...
    publish();
    try {
      const exitCode = await resumePrompt(
        runParsed,
        cwd,
        env,
        io,
  ...
  const controller: InkTuiController = {
    ...
    async submit(input: string) {
      ...
      if (input === "/status") {
        setRuntimeState();
        notify(`provider=${state.runtime.provider} model=${state.runtime.model} approval=${state.runtime.approvalMode} session=${state.sessionContext.activeSessionId ?? "none"} latest=${state.sessionContext.lastSessionId ?? "none"} resumedFrom=${state.sessionContext.resumedFromSessionId ?? "none"} context=${state.sessionContext.historyMessageCount}`);
        return;
      }
      if (input === "/provider") {
        setRuntimeState();
      ...
      if (input === "/tools") {
        ...
        const tools = Array.from(selectTools(runtimeConfig, await createCliRuntimeTools(runtimeConfig, env)).keys()).sort();
        notify(tools.length === 0 ? "tools: none" : `tools: ${tools.join(", ")}`);
        return;
      }
      if (input === "/session") {
        notify(`session=${state.sessionContext.activeSessionId ?? chatSessionId} latest=${state.sessionContext.lastSessionId ?? "none"} resumedFrom=${state.sessionContext.resumedFromSessionId ?? "none"} status=${state.status} turns=${state.turns} context=${state.sessionContext.historyMessageCount}`);
        return;
      }
      ...
      if (command?.name === "plan" || command?.name === "work") {
        ...
        await runTuiAgentPrompt({
          ...
          command: mode,
          prompt
        }, input, prompt, chatHistoryMessages);
        return;
      }
      if (command?.name === "resume") {
...
export async function runCli(argv: string[], io: CliIo = {}): Promise<number> {
  ...
  try {
    ...
    if (parsed.command === "plan" || parsed.command === "work") {
      if (!parsed.prompt) {
        emitStderr(io, `error: Command "${parsed.command}" requires a prompt. Run "xenesis ${parsed.command} --help" for usage.`);
        return 1;
      }
      return await runPrompt(parsed, cwd, env, io, parsed.prompt, (writer) => {
        sessionWriter = writer;
      }, [], { abortSignal: io.abortSignal });
    }
