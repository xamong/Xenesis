...
export const localReviewCommandNames = ['advisor', 'agents'] as const;
...
const commandSpecs: Record<ReviewPromptCommandName, ReviewCommandSpec> = {
  ...
  'perf-issue': {
    ...
    objective:
      'Investigate the supplied performance issue details and local changes for likely bottlenecks, regressions, and measurement gaps.',
    focus: [
  ...
  'autofix-pr': {
    ...
    objective:
      'Use supplied PR feedback and local changes to plan or perform bounded local fixes through the normal Xenesis agent path.',
    focus: [
  ...
  issue: {
    ...
    objective:
      'Analyze the supplied issue text against the local repository and produce a concrete reproduction, impact, and implementation path.',
    focus: [
    ...
    output: ['Issue summary and assumptions.', 'Reproduction or diagnostic plan.', 'Implementation plan and tests.'],
  },
  'pr-comments': {
...
const commandNameSet = new Set<string>([...reviewPromptCommandNames, ...localReviewCommandNames]);
...
async function readGitContext(workspace: string): Promise<GitContext> {
  ...
  const diffBase = mergeBase.ok && mergeBase.stdout.trim() ? mergeBase.stdout.trim() : 'HEAD';
...
export async function buildReviewCommandPrompt(command: ReviewPromptCommandName, workspace: string, args?: string) {
  ...
  const userInput = trimmedArgs ? trimmedArgs : '(none supplied)';
...
export function renderAgentsCommand(config: XenesisConfig, args?: string) {
  ...
  const definitions = Object.entries(config.extensions.subagents.definitions).sort(([left], [right]) =>
    left.localeCompare(right),
  );
  const filtered = selector && selector !== 'list' ? definitions.filter(([name]) => name === selector) : definitions;
  ...
  for (const [name, definition] of filtered) {
    lines.push(
      `agent: ${name} approvalMode=${definition.approvalMode ?? 'default'} maxTurns=${definition.maxTurns ?? 'default'} tools=${definition.tools?.join(',') ?? 'all'}`,
    );
  }
