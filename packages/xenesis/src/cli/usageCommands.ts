...
export async function renderLoginStatusCommand(config: XenesisConfig, env: NodeJS.ProcessEnv) {
  ...
  const apiKeyStatus = settings.apiKeyEnv
    ? (settings.apiKey && settings.apiKey.trim().length > 0 ? "present" : "missing")
    : "not_required";
  return [
