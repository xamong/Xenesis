...
import { Box, Text, render, useApp, useInput, useStdin, useStdout, useWindowSize, type RenderOptions } from "ink";
import {
...
function TuiApp({ controller, inputHistory = [] }: InkTuiAppProps) {
  ...
  const view = useMemo(() => createTuiViewModel(state, {
    width: size.columns || 80,
    height: size.rows || 24
  }, { inputValue: rawInput, scrollbackOffset }), [state, size.columns, size.rows, rawInput, scrollbackOffset]);
  ...
  return React.createElement(
    ...
    view.commandOutput?.expanded
      ? React.createElement(
        ...
        React.createElement(SidePanel, { tools: view.toolRows, commandOutput: undefined, notices: view.noticeRows, approval: view.approval })
      )
    ...
    React.createElement(Footer, { footer: view.footer, input: renderTuiInputWithCursor(input), rawInput, busy, suggestions: view.suggestionRows })
  );
}
...
function CommandOutputPanel({ output }: { output: NonNullable<TuiViewModel["commandOutput"]> }) {
  return React.createElement(
    ...
    React.createElement(Text, { color: output.kind === "error" ? "red" : "cyan", wrap: "wrap" }, `${output.command} (${output.range})`),
    ...(output.lines.length === 0
      ? [React.createElement(Text, { key: "output-empty", color: "gray" }, "No output.")]
      : output.lines.map((line, index) => React.createElement(
        Text,
        { key: `expanded-output-${index}`, color: output.kind === "error" ? "red" : "white", wrap: "wrap" },
        line
      ))),
    ...(output.savedPath ? [React.createElement(Text, { key: "output-saved", color: "gray", wrap: "wrap" }, `saved ${output.savedPath}`)] : [])
  );
}
...
function SidePanel({
  ...
  return React.createElement(
    ...
    ...(tools.length === 0
      ...
      : tools.map((tool, index) => React.createElement(
        Text,
        { key: `tool-${index}`, color: tool.status === "failed" ? "red" : tool.status === "completed" ? "green" : "yellow", wrap: "wrap" },
    ...
    ...(commandOutput ? [
      ...
      React.createElement(Text, { key: "command-output-command", color: commandOutput.kind === "error" ? "red" : "cyan", wrap: "wrap" }, `${commandOutput.command} (${commandOutput.range})`),
      ...commandOutput.lines.map((line, index) => React.createElement(
        Text,
        { key: `command-output-${index}`, color: commandOutput.kind === "error" ? "red" : "gray", wrap: "wrap" },
        line
      )),
      ...(commandOutput.savedPath ? [React.createElement(Text, { key: "command-output-saved", color: "gray", wrap: "wrap" }, `saved ${commandOutput.savedPath}`)] : [])
    ] : []),
    ...(approval ? [
      React.createElement(Text, { key: "approval-title", bold: true }, "Approval"),
      React.createElement(Text, { key: "approval-name", color: "yellow", wrap: "wrap" }, `${approval.name} (${approval.riskLevel})`),
    ...
    ...(notices.length === 0
      ...
      : notices.map((notice, index) => React.createElement(
        Text,
        { key: `notice-${index}`, color: notice.kind === "error" ? "red" : notice.kind === "warning" ? "yellow" : "gray", wrap: "wrap" },
        notice.message
      )))
  );
}
  ...
  return React.createElement(
    ...
    ...(suggestions.length > 0 ? [
      React.createElement(
        ...
        `Suggestions: ${suggestions.map((suggestion) => suggestion.command
          ? `${suggestion.usage} - ${suggestion.description}`
          : suggestion.description).join("  |  ")}`
      )
    ] : []),
    React.createElement(Text, { color: busy ? "yellow" : "white" }, `${busy ? "running" : "tui"}> ${rawInput ? input : ""}`)
  );
}
