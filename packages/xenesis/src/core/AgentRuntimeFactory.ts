...
export function createProvider(
  ...
  if (selectedProvider === 'codex-responses' && !getProviderFactory('codex-responses')) {
    registerCodexResponsesProvider();
  }
  ...
  if (reg) {
    const regSettings = resolveProviderSettings(selectedConfig, env);
    return withProviderModel(
      reg({
        name: selectedProvider,
        model: selectedModel,
        apiKey: selection.apiKeyEnv ? env[selection.apiKeyEnv] : regSettings.apiKey,
        baseURL: selection.baseURL ?? regSettings.baseURL,
        env,
      }),
      selectedModel,
    );
  ...
  if (selectedProvider === 'codex-app-server')
    return withProviderModel(new CodexAppServerProvider({ env }), selectedModel);
  if (selectedProvider === 'codex-cli') return withProviderModel(new CodexCliProvider({ env }), selectedModel);
  if (selectedProvider === 'claude-interactive')
    return withProviderModel(new ClaudeInteractiveProvider({ env }), selectedModel);
  ...
  const apiKey =
    settings.apiKey ??
    ((capabilitiesFor(settings.provider) ?? PROVIDER_CAPABILITIES[settings.provider]).requiresApiKey
      ? undefined
      : 'xenesis-local');
  if (apiKey === undefined) {
...
function candidateKey(candidate: FallbackCandidate, fallbackModel: string) {
  return [candidate.kind, fallbackModel, candidate.baseURL ?? '', candidate.apiKeyEnv ?? ''].join('\0');
}
...
function fallbackRequiresCredential(candidate: FallbackCandidate) {
  return (
    (capabilitiesFor(candidate.kind) ?? PROVIDER_CAPABILITIES[candidate.kind]).requiresApiKey ||
    Boolean(candidate.apiKeyEnv)
  );
}
...
export function resolveFallbackChain(config: XenesisConfig, env: NodeJS.ProcessEnv): FallbackChainResult {
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.kind === 'mock' && env.XENESIS_ENABLE_TEST_MOCK_PROVIDER !== 'true') {
      skip('no-credential');
      continue;
    }
    ...
    chain.push({
      ...
      supportsTools:
        ...
        (capabilitiesFor(candidate.kind) ?? PROVIDER_CAPABILITIES[candidate.kind]).supportsTools,
      label,
    });
  }
...
export async function createRuntimeTools(
  ...
  if (/^(1|true|yes)$/iu.test(env.XENESIS_SESSION_SEARCH ?? '')) {
    ...
    if (registry.has(sessionSearchTool.name))
      throw new Error(`Tool "${sessionSearchTool.name}" is already registered.`);
    ...
    sources: [
      {
        ...
        detail:
          config.extensions.skills.paths.length > 0
...
async function createMemorySystemContext(config: XenesisConfig, prompt: string): Promise<RunSystemContext> {
  if (!config.extensions.memory.enabled) {
    return {
      messages: [],
      sources: [
...
async function createWorkspaceContextSystemContext(config: XenesisConfig, prompt: string): Promise<RunSystemContext> {
...
async function createAgentMessageSystemContext(
  ...
  const summary = await collectAgentMessages(messageStore, options.taskId, {
...
async function releaseClaimedAgentMessagesForContext(
  ...
  await new SqliteAgentMessageStore({ xenesisHome: config.xenesisHome }).releaseClaims(
    [...messageIds],
    receiverSessionId,
  );
}
...
async function createSavedPlanSystemContext(
  ...
  return {
    messages: [
      {
        role: 'system',
        content: ['Xenesis loaded plan:', '', plan.trimEnd()].join('\n'),
      },
    ],
    sources: [
      {
...
function splitBaseRuntimePrompt(baseMessages: readonly SystemMessage[]) {
  ...
  for (const line of content.split('\n')) {
    ...
    if (/large files|large source files|truncated|startLine\/maxLines|diagnostics/i.test(line)) {
...
function dynamicContextSources(
  ...
  for (const drop of built.audit.dropped) {
    const adapterId = drop.id.startsWith('instruction:') ? 'instruction-discovery' : (drop.id.split(':')[0] ?? drop.id);
    droppedByAdapter.set(adapterId, drop.reason);
  }

  return built.audit.sourceAdapters.map((adapterId) => {
    const injected = adapterId === 'instruction-discovery' ? instructionSelected : selectedAdapterIds.has(adapterId);
    ...
    return {
      ...
      ...(!injected && droppedByAdapter.has(adapterId) ? { droppedReason: droppedByAdapter.get(adapterId)! } : {}),
    };
  });
}
...
export async function createRunSystemContext(
  ...
  const mcpServerCount =
    Object.keys(config.extensions.mcpServers).length + (config.extensions.recommendedMcpServers?.length ?? 0);
  ...
  const adapters: ContextSourceAdapter[] = [
    staticRecordAdapter({
      id: 'ide',
      kind: 'ide_context',
      authority: 'active_surface',
      cacheScope: 'session',
      priority: 1010,
      messages: ide.messages,
    }),
    createInstructionContextAdapter({
      workspaceRoot: config.workspace,
      cwd: options.cwd ?? config.workspace,
      tokenPriorityBase: 1015,
    }),
    staticRecordAdapter({
      id: 'saved_plan',
      kind: 'session_memory',
    ...
    staticRecordAdapter({
      ...
      kind: 'workspace_context',
      authority: 'workspace_index',
      cacheScope: 'session',
      priority: 1040,
      messages: workspaceContext.messages,
    }),
    staticRecordAdapter({
      id: 'background_task',
      kind: 'session_memory',
      ...
      cacheScope: 'session',
      priority: 1050,
      messages: backgroundTasks.messages,
    }),
    staticRecordAdapter({
      id: 'agent_message',
      kind: 'session_memory',
    ...
    staticRecordAdapter({
      ...
      cacheScope: 'session',
      priority: 1060,
      messages: operationalFailures.messages,
    }),
  ];
  ...
  return {
    messages: [section13SystemMessage],
    sources: [
      {
        source: 'base',
...
export async function saveLatestPlan(config: XenesisConfig, content: string, onNotice?: RuntimeNoticeHandler) {
