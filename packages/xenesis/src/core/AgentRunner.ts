...
function providerRawRecordsFromCliRaw(raw: unknown): Record<string, unknown>[] {
  const turnItemsFromRawRecord = (record: Record<string, unknown>): Record<string, unknown>[] => {
    const params = plainRecord(record.params);
    const turns = [
      plainRecord(record.turn),
      plainRecord(plainRecord(record.initial)?.turn),
      plainRecord(plainRecord(record.completed)?.turn),
      plainRecord(params?.turn),
    ];
    return turns.flatMap((turn) => {
      const items = turn?.items;
      return Array.isArray(items)
        ? items.filter((item): item is Record<string, unknown> => Boolean(plainRecord(item)))
        : [];
    });
  };
  const appendRecord = (out: Record<string, unknown>[], value: unknown) => {
    const record = plainRecord(value);
    if (!record) return;
    out.push(record, ...turnItemsFromRawRecord(record));
  };
  if (Array.isArray(raw)) {
    const out: Record<string, unknown>[] = [];
    for (const item of raw) appendRecord(out, item);
    return out;
  }
  ...
  if (Array.isArray(records)) {
    for (const item of records) appendRecord(out, item);
  }
...
function providerRawRecordIsCompleted(record: Record<string, unknown>) {
  const params = plainRecord(record.params);
  const item = plainRecord(params?.item) ?? (stringValue(record, 'type') ? record : undefined);
  const method = stringValue(record, 'method');
  const statusText = [stringValue(record, 'status'), stringValue(params, 'status'), stringValue(item, 'status')].join(
    ' ',
  );
  return (
...
function providerRawRecordLooksLikeDeskMcpToolCall(record: Record<string, unknown>) {
  ...
  const searchable = JSON.stringify({
    ...
    itemServer: stringValue(item, 'server'),
    itemTool: stringValue(item, 'tool'),
