...
import { type XenesisConfig, xenesisStatePath } from '../config/index.js';
...
function createOperationalReportItem(
  fileName: string,
  report: Record<string, unknown>,
): OperationalFailureContextItem | undefined {
...
function isFailedRunReport(report: Record<string, unknown>) {
  ...
  const status = optionalText(report.status);
  return success === false || status === 'failed' || status === 'error' || failedToolSummaries(report).length > 0;
}

function createRunReportFailureItem(
  fileName: string,
  report: Record<string, unknown>,
): OperationalFailureContextItem | undefined {
...
function createTaskFailureItem(task: Record<string, unknown>): OperationalFailureContextItem | undefined {
  ...
  return {
    ...
    details: [...(prompt ? [`prompt: ${prompt}`] : []), ...(reason ? [`reason: ${reason}`] : [])],
  };
}
...
async function collectFailedTasks(config: XenesisConfig, maxItems: number) {
  if (maxItems <= 0) return [];
  const tasks = (await new SqliteAgentTaskStore({ xenesisHome: config.xenesisHome }).list()) as unknown as Record<
    string,
    unknown
  >[];
  return tasks
...
async function readRunReportRecords(config: XenesisConfig, maxItems: number) {
  ...
  return records
    .filter((record): record is { fileName: string; report: Record<string, unknown>; sortTime: number } =>
      Boolean(record),
    )
...
function isBlockedRepairTrace(decision: Record<string, unknown>) {
  ...
  return optionalText(decision.status) === 'blocked' || title.includes('blocked') || reason.includes('blocked');
}
...
function summarizePatternRepairStops(records: Array<{ report: Record<string, unknown> }>) {
  ...
  for (const { report } of records) {
    ...
    for (const decision of decisions.filter(isObject)) {
      ...
      summary.count += 1;
      summary.failedCommands = uniqueInOrder([...summary.failedCommands, ...stringArray(decision.related)]);
      summary.latestSessionId ??= optionalText(report.sessionId);
      summary.latestTraceId ??= optionalText(report.traceId);
...
function capabilityImpactFromResult(result: Record<string, unknown>) {
  ...
  return {
    ...
    verification: uniqueInOrder([...stringArray(impact.verification), ...stringArray(result.verification)]),
...
function capabilityImpactEffectiveness(
  ...
  const qualityScoreDelta = roundedDelta(averageMetric(after, 'qualityScore'), averageMetric(before, 'qualityScore'));
...
export async function collectOperationalFailureContext(
  config: XenesisConfig,
): Promise<OperationalFailureContextSummary> {
  ...
  const items = [...reports, ...runReports, ...tasks].sort(compareOperationalFailures).slice(0, policy.maxItems);
  ...
  const baseDetail =
    items.length > 0 ? items.map((item) => `${item.kind}:${item.id}`).join(', ') : 'no recent failed reports or tasks';
  return {
    enabled: true,
    total: items.length,
    ...
    items,
    adaptivePolicy,
    detail: adaptivePolicy.active ? `${baseDetail}; adaptive policy active: ${adaptivePolicy.detail}` : baseDetail,
  };
}
...
export function operationalFailureContextSource(
  summary: OperationalFailureContextSummary,
): Omit<ContextSourceEvent, 'type'> {
  return {
...
function hasRecentShellFailures(summary: OperationalFailureContextSummary) {
  return summary.items.some((item) =>
    /\bshell\s+\d+\/\d+\s+failures\b/i.test([item.summary, ...item.details].join('\n')),
  );
}
...
function operationalRecoveryPolicyLines(summary: OperationalFailureContextSummary) {
  ...
  return ['', 'Operational recovery policy:', ...rules.map((rule) => `- ${rule}`)];
}
