import {
  auditCompletionClaims,
  type ClaimAuditReport,
  type CompletionClaim
} from "./ClaimAuditor.js";
...
export function evaluateCompletionGate(input: CompletionGateInput): CompletionGateReport {
  ...
  if (pendingApprovalCount > 0) {
    return {
      ...
      reasons: [countReason(pendingApprovalCount, "pending approval requires user input", "pending approvals require user input")],
      claimAudit: emptyClaimAudit(input)
    };
  }
  ...
  if (unresolvedRequiredTaskCount > 0) {
    return {
      ...
      reasons: [countReason(unresolvedRequiredTaskCount, "required durable task is unresolved", "required durable tasks are unresolved")],
      claimAudit: emptyClaimAudit(input)
    };
  }
  ...
  if (!claimAudit.ok) {
    return {
      ...
      reasons: [`unsupported completion claims: ${claimAudit.unsupportedClaims.map((claim) => claim.reason).join("; ")}`],
      claimAudit
    };
  }
