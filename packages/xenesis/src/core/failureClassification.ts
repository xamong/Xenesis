import { type FailureRepairRecipe, getFailureRepairRecipe } from './repairRecipes.js';
...
const fallbackClassification: VerificationFailureClassification = {
  ...
  repairPlan:
    'Inspect the failed verification output, make the smallest concrete fix, then rerun the failed verification.',
...
function hasForbiddenRenderedValue(content: string) {
  return includesAny(content, [/forbidden_text/i, /\bundefined\b/i, /\bNaN\b/i, /\[object Object\]/i]);
}
...
export function classifyVerificationFailure(input: VerificationFailureInput): VerificationFailureClassification {
  ...
  if (
    ...
    includesAny(content, [/Only\s+HTTP\(S\)\s+URLs\s+are\s+allowed/i, /file:\/\//i, /Invalid URL/i])
  ) {
  ...
  if (
    ...
    includesAny(content, [
      /http_status_error/i,
      /http_error_page/i,
      /server_default_page/i,
      /httpStatus:\s*[45]\d\d/i,
      /HTTP\s*(?:오류|Error)\s*[45]\d\d/i,
      /IIS\s+Windows\s+Server/i,
    ])
  ) {
    return createClassification(
      {
        ...
        evidence: compactEvidence(
          content,
          /http_status_error|http_error_page|server_default_page|httpStatus:\s*[45]\d\d|HTTP\s*(?:오류|Error)\s*[45]\d\d|IIS\s+Windows\s+Server/i,
        ),
      },
      ['app_launch_plan', 'server', 'app_e2e_check'],
  ...
  if (
    includesAny(content, [/page_error/i, /client_script_syntax_error/i, /Uncaught SyntaxError/i, /Unexpected token/i])
  ) {
    return createClassification(
      {
        ...
        evidence: compactEvidence(
          content,
          /page_error|client_script_syntax_error|Uncaught|SyntaxError|Unexpected token/i,
        ),
      },
    ...
    return createClassification(
      {
        ...
        evidence: compactEvidence(
          content,
          /timedOut|timeoutHint|readiness.*timed out|open handles|child server cleanup/i,
        ),
      },
  ...
  if (input.toolName === 'app_e2e_check' && hasForbiddenRenderedValue(content)) {
    return createClassification(
      {
  ...
  if (
    includesAny(content, [
      /missing_expected_text/i,
      /expectedTextMissing:/i,
      /text_below_min/i,
      /interactive_elements_below_min/i,
      /title_missing/i,
    ])
  ) {
    return createClassification(
      {
        ...
        evidence: compactEvidence(
          content,
          /missing_expected_text|expectedTextMissing|text_below_min|interactive_elements_below_min|title_missing/i,
        ),
      },
      ['read', 'patch', 'app_readiness', 'app_e2e_check'],
  ...
  if (
    includesAny(content, [
      /unbounded_readiness_polling/i,
      /http_response_not_consumed/i,
      /failure_exit_missing/i,
      /startup_failure_can_hang/i,
    ])
  ) {
    return createClassification(
      {
        ...
        evidence: compactEvidence(
          content,
          /unbounded_readiness_polling|http_response_not_consumed|failure_exit_missing|startup_failure_can_hang/i,
        ),
      },
...
export function renderVerificationFailureClassification(classification: VerificationFailureClassification) {
  ...
  if (classification.primaryCause) {
    ...
    if (classification.primaryCause.evidence)
      lines.push(`failureCauseEvidence: ${classification.primaryCause.evidence}`);
  }
