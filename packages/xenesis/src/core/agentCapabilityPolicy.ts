...
export function mergeToolExecutionPolicies(
  ...
  return {
    name: active.map((policy) => policy.name).filter(Boolean).join("+") || "merged",
...
export function mergeToolGuardConfigs(
  base: ToolGuardConfig,
  override?: Partial<ToolGuardConfig>
): ToolGuardConfig {
  if (!override) return {
  ...
  return [
    {
      ...
      objective: "Exercise representative workspace, edit, verification, Desk, current-info, provider, and channel tasks.",
      ...
      preferredTools: ["scenario", "smoke", "run_reports", "tree", "glob", "list", "read", "search", "code_symbols", "lsp", "diagnostics", "app_launch_plan", "app_readiness", "app_e2e_check", "weather_current", "weather_forecast", "market_quote", "sports_scores", "news_latest", "web_fetch", "desk_state", "desk_active_context", "desk_context_actions", "desk_capabilities", "agent_task", "todo"]
    },
    {
      id: "self_check_loop",
      ...
      preferredTools: ["todo", "diagnostics", "run_reports", "tree", "glob", "list", "read", "search", "code_symbols", "lsp", "diff", "patch", "json"]
    },
    {
      ...
      preferredTools: ["tree", "glob", "list", "read", "search", "code_symbols", "lsp", "diff", "patch", "json", "diagnostics", "app_launch_plan", "app_readiness", "app_e2e_check"]
    },
    {
      ...
      preferredTools: ["desk_state", "desk_active_context", "desk_context_actions", "workspace_context", "memory", "agent_task"]
    },
    {
    ...
    },
    {
      ...
      preferredTools: ["desk_state", "desk_active_context", "desk_context_actions", "desk_call_capability", "desk_recent_diagnostics", "desk_capabilities"]
    }
  ];
}
...
export function classifyAgentScenario(prompt: string): ClassifiedAgentScenario {
  ...
  const matched = scenarios.find((scenario) => (
    scenario.promptSignals.some((signal) => normalized.includes(signal.toLowerCase()))
  )) ?? scenarios[0];
  return {
...
export function buildAgentSelfCheckLoop(input: {
  fileChangesExpected?: boolean;
  ...
  const steps: AgentSelfCheckStep[] = [
    ...
    { id: "verify", purpose: "run focused checks or inspect result state", requiredWhen: input.verificationAvailable ? "available" : "when evidence exists" },
    { id: "repair_once", purpose: "repair from concrete failure evidence", requiredWhen: input.fileChangesExpected ? "after failed verification" : "only when safe" },
...
export function buildProviderQualityMetrics(records: SessionLogRecord[]): ProviderQualityMetrics {
  ...
  const riskLevel = fallbacks.length > 1 || retries.length >= 3 || contextRecoveries.length > 1
    ? "high"
