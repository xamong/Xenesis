...
export type DiagnosticCapabilityImpactEffectivenessStatus = 'improved' | 'regressed' | 'mixed' | 'insufficient_data';
...
function recoveryActions(input: {
  ...
  return uniqueInOrder([
    ...
    ...(input.toolChoicePatterns?.some((pattern) => pattern.missedTool === 'shell')
      ? ['prefer_structured_tools_before_shell']
      : []),
    ...(input.capabilityPolicyImpacts?.some((pattern) => pattern.area === 'tool-selection-policy')
      ? ['prioritize_structured_tools_before_generic_answers']
      : []),
  ]);
}

function mergeRequiredBefore(base: Record<string, string[]> | undefined, adaptive: Record<string, string[]>) {
...
export function buildAdaptiveExecutionPolicy(patterns: DiagnosticFailurePatterns): AdaptiveExecutionPolicy {
  ...
  const cautionTools = uniqueInOrder([
    ...
    ...toolRecoveryPatterns.filter((pattern) => pattern.unrecoveredCount > 0).flatMap((pattern) => pattern.tools),
  ...
  return {
    ...
    externalStrategy: {
      statusFields: ['diagnostics.failurePatterns', 'diagnostics.adaptivePolicy', 'tasks', 'traces', 'observability'],
    ...
    detail: active
      ? [
          ...
          toolRecoveryPatterns.length > 0
            ? `toolRecovery=${toolRecoveryPatterns.map((pattern) => pattern.kind).join(',')}`
            : '',
          toolChoicePatterns.length > 0
            ? `toolChoice=${toolChoicePatterns.map((pattern) => `${pattern.priorityReason}:${pattern.missedTool}`).join(',')}`
            : '',
          capabilityPolicyImpacts.length > 0
            ? `capabilityImpact=${capabilityPolicyImpacts.map((pattern) => pattern.area).join(',')}`
            : '',
...
export function adaptiveExecutionPolicySystemLines(policy: AdaptiveExecutionPolicy) {
  ...
  return ['', 'Adaptive execution policy:', ...policy.rules.map((rule) => `- ${rule}`)];
}
