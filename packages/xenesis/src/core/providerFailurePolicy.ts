...
export function classifyProviderFailure(error: unknown): ProviderFailureClassification {
  ...
  if (error instanceof ProviderHttpError) {
    if (error.status === 429) return { kind: "rate_limit", message: error.message, retryable: true, fallbackRecommended: true };
    if (error.status === 529 || error.status === 503) return { kind: "overloaded", message: error.message, retryable: true, fallbackRecommended: true };
    if (error.status === 408) return { kind: "timeout", message: error.message, retryable: false, fallbackRecommended: true };
    if (error.status === 401 || error.status === 403) return { kind: "auth", message: error.message, retryable: false, fallbackRecommended: false };
  }
...
export function decideProviderAttempt(input: ProviderAttemptDecisionInput): ProviderAttemptDecision {
  ...
  const eligibleOffset = input.remainingProviderSupportsTools.findIndex((supportsTools) =>
    !input.toolsRequired || supportsTools
  );
  if (eligibleOffset >= 0) {
...
export function computeRetryDelayMs(input: {
  ...
  const exponential = input.baseDelayMs * Math.pow(2, input.attempt);
