...
function modeDefaultDecision(
  ...
  if (normalizedMode === "acceptEdits") {
    if (operation.isReadOnly) {
      return decision("allow", "acceptEdits mode allows read-only operations", "mode:acceptEdits", normalizedMode, "low");
    }
    ...
    if (operation.operationKind === "shell") {
      return decision("ask", "acceptEdits mode requires approval for shell", "mode:acceptEdits", normalizedMode, "medium");
    }
    return decision("ask", "acceptEdits mode requires approval for non-file operations", "mode:acceptEdits", normalizedMode, "medium");
  }
  ...
  if (normalizedMode === "auto") {
    ...
    return decision("ask", "auto mode requires approval for ambiguous operations", "mode:auto", normalizedMode, "medium");
  }

  if (normalizedMode === "bypassPermissions") {
    if (operation.isReadOnly || operation.bypassAllowed) {
      return decision("allow", "bypassPermissions mode allowed by policy", "mode:bypassPermissions", normalizedMode, "medium");
    }
    return decision("ask", "bypassPermissions mode requires explicit bypass allowance", "mode:bypassPermissions", normalizedMode, "medium");
  }
  ...
  return decision("ask", "default mode requires approval for unsafe operations", "mode:default", normalizedMode, "medium");
}

function applyDontAsk(
  normalizedMode: PermissionMode,
  candidate: PermissionEngineDecision
): PermissionEngineDecision {
