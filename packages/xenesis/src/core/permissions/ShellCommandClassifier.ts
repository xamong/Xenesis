...
const networkFetchCommands = new Set([
  "curl",
  "wget",
  "invoke-webrequest",
  "iwr",
...
function powershellFinding(command: string, tokens: string[]) {
  ...
  if (/\bStart-Process\b/i.test(command) && /-Verb\s+RunAs\b/i.test(command)) return "PowerShell elevation via Start-Process -Verb RunAs";
...
function pipeToInterpreterFinding(command: string) {
  ...
  if (
    downloader.test(command) &&
    /\$\(|`/.test(command) &&
    new RegExp(`\\b(?:${interpreter})\\b`, "i").test(command)
  ) {
...
function classifyShellCommandInternal(input: ShellCommandClassifierInput, depth: number): ShellCommandClassifierResult {
  ...
  const finding = destructiveGitFinding(tokens)
