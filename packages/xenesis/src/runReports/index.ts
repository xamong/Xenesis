...
import {
  buildProviderQualityMetrics,
  type ProviderQualityMetrics
} from "../core/agentCapabilityPolicy.js";
...
function terminalStatus(records: SessionLogRecord[]) {
  ...
  const stopped = records.find((record): record is SessionLogRecord & { type: "stopped"; turns: number } => (
    record.type === "stopped"
  ));
  if (stopped) return { status: "stopped", turns: stopped.turns };
  const done = records.find((record): record is SessionLogRecord & { type: "done"; turns: number } => (
    record.type === "done"
  ));
...
function handoffTaskLabel(task: Record<string, unknown>) {
  ...
  return optionalString(task.label) ??
...
function summarizeToolPolicySnapshots(records: SessionLogRecord[]): RunReportToolPolicySnapshot[] {
  return records
    .filter((record): record is SessionLogRecord & { type: "tool_policy_snapshot" } => record.type === "tool_policy_snapshot")
    .map((record) => {
      const {
        type: _type,
        sessionId: _sessionId,
...
function findRecoveryForCandidate(
  ...
  const targetNames = candidate.recoveryTargets.length > 0
    ? new Set(candidate.recoveryTargets)
    : candidate.kind === "missing_tool"
      ? undefined
      : new Set([candidate.toolName]);

  return successes.find(({ record, index }) => (
    index > candidate.index &&
    (!targetNames || targetNames.has(record.message.name))
  ));
}
...
function publicToolRecoveryItem(candidate: ToolRecoveryCandidate): RunReportToolRecoveryItem {
  const {
    index: _index,
    recoveryTargets: _recoveryTargets,
    ...item
  } = candidate;
  return item;
}
...
function summarizeDecisionTrace(
  ...
  for (const source of contextSources.filter((item) => item.injected)) {
    decisions.push({
      ...
      related: [
        source.name,
        ...(source.itemCount !== undefined ? [`items:${source.itemCount}`] : [])
      ]
    });
  }
  ...
  for (const audit of toolPolicyAudits) {
    decisions.push({
      ...
      status: audit.status,
      related: audit.priorityTools && audit.priorityTools.length > 0
        ? audit.priorityTools
...
function buildMetrics(
  ...
  const stoppedReasons = records.filter((record): record is SessionLogRecord & { type: "stopped" } => record.type === "stopped");
  ...
  const permissionDenyCount = permissionAudits.filter((record) =>
    record.type === "permission_audit" && record.status === "deny").length;
  const toolPolicyDenyCount = toolPolicyAudits.filter((record) => record.status === "deny").length;
  const repairAttemptCount = repairs?.length ?? repairDecisions.filter((record) => (
    record.type === "repair_decision" && (record.status === "continue" || record.status === "completed")
  )).length;
  ...
  const providerDegraded = providerQuality.retryCount > 0 ||
    providerQuality.fallbackCount > 0 ||
    providerQuality.contextRecoveryCount > 0;
  ...
  return {
    ...
    ...(verification ? {
      ...
      verificationPassRate: verificationTotal && verificationTotal > 0 ? verification.passed / verificationTotal : 0
    } : {}),
    repairAttemptCount,
    repairSuccess,
    toolFailureRate,
...
function contentHasFailureSignal(content: string) {
  return /verificationOk:\s*false|repairRequired:\s*true|timedOut:\s*true|failureLikeOutput:\s*true|completionBlocked:\s*true|status:\s*(?:fail|failed)|failures:\s*[1-9]\d*|pageErrors:\s*[1-9]\d*|exitCode:\s*(?:none|[1-9]\d*)|tool policy denied|failed:|error:|net::ERR_|page\.goto/i.test(content);
}
...
function inferVerificationReportFromToolResults(records: SessionLogRecord[]): VerificationReport | undefined {
  const verificationResults = records
    .filter((record): record is SessionLogRecord & { type: "tool_result" } => (
...
function buildRunSelfReview(input: {
  status: string;
  metrics: RunReportMetrics;
}): RunReportSelfReview {
  ...
  const nextActions = uniqueActions(findings.map((finding) => {
    if (finding.area === "completion") return "resume_incomplete_run";
    if (finding.area === "verification") return metrics.verificationAfterChangeMissing
      ? "run_verification_after_change"
  ...
  const status = !metrics.success || hasHigh
    ? "fail"
    : findings.length > 0 || metrics.qualityScore < 100
      ? "warn"
      : "pass";
