...
async function fetchRemoteSessionEvents<TEvent>(
  ...
  const response = await source.fetchPage({
...
function lastContent(records: SessionLogRecord[], type: "user_message" | "assistant_message") {
  const record = records
    .filter((candidate): candidate is SessionLogRecord & {
      type: typeof type;
...
export function hasApprovalResolved(records: SessionLogRecord[], toolCallId: string): boolean {
  return records.some(
    (record) => record.type === "approval_resolved" &&
      (record as { toolCallId?: string }).toolCallId === toolCallId
  );
}
...
export function compactSessionEvents(records: SessionLogRecord[]) {
  ...
  const followedToolChoiceCount = toolChoiceAudits.filter((record) =>
    record.type === "tool_choice_audit" && record.status === "followed_priority").length;
  const missedToolChoiceCount = toolChoiceAudits.filter((record) =>
    record.type === "tool_choice_audit" && record.status === "missed_priority").length;
  const state = latestRunState(records);
  const artifact = records.filter((record): record is SessionLogRecord & {
    type: "artifact";
    artifactId: string;
    title: string;
  } => record.type === "artifact").at(-1);
  const done = records.find((record): record is SessionLogRecord & { type: "done"; content: string } => (
    record.type === "done"
  ));
  const selfReview = records.find((record): record is SessionLogRecord & {
