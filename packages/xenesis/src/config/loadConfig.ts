...
export const mcpServerConfigSchema = z.preprocess(
  (v) => {
    ...
    if (record.transport === "http" || record.transport === "sse" || (record.url !== undefined && record.command === undefined)) {
...
const operationalFailureContextSchema = z.object({
...
const permissionsSchema = z.object({
  ...
  toolPolicies: z.record(z.object({
...
const configSchema = z.object({
  ...
  providerFallbacks: z.array(z.object({
...
async function readConfigFile(
  ...
  const resolvedPath = configPath
    ...
    : resolve(cwd, "xenesis.config.json");
...
export async function loadConfig(options: LoadConfigOptions): Promise<XenesisConfig> {
  ...
  const merged = configSchema.parse(mergeConfigLayers(
    defaultConfig,
    fileConfig,
    profileConfig,
    envConfigOverrides,
