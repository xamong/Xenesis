...
async function checkToolCapabilities() {
  ...
  try {
    ...
    const shell = await tools.get("shell")!.run({
      command: "node -e \"require('fs').writeFileSync('blocked-marker.txt','ran')\"; Remove-Item -Recurse missing-target",
      timeoutMs: 5000
    }, context);
...
async function checkAgent(options: RunSmokeOptions) {
  ...
  const content = events
    .map((event) => event.content ?? event.message?.content ?? "")
    .join("\n");
...
function renderSmokeLines(report: SmokeReport, xenesisHome: string, reportPath: string) {
  ...
  return [
    ...
    ...report.checks.map((check) => check.status === "passed"
      ? `smoke: ${check.name} passed`
      : `smoke: ${check.name} failed - ${check.message}`)
  ];
}
...
export async function readSmokeReportEntry(
  xenesisHome: string,
  target: string
): Promise<SmokeReportEntry | undefined> {
