import { createLlmOrchestrationMetadata, llmOrchestrationSystemLines } from './llmOrchestrationPolicy.js';
...
export function createXenisSystemMessage(env: NodeJS.ProcessEnv | undefined): WorkflowSystemMessage {
  ...
  return {
    ...
    content: [
      ...
      hasBridgeToken
        ? 'Xenesis Desk MCP bridge token is configured. Do not reveal it.'
        : 'Xenesis Desk MCP bridge token is not configured.',
    ].join('\n'),
  };
}
