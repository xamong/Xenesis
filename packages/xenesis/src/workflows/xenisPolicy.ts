...
export type XenisTaskPolicyId = 'xd-command' | 'desk-general';
...
export function resolveXenisTaskPolicy(prompt: string): XenisTaskPolicy {
  ...
  if (lowerPrompt === '/xd' || lowerPrompt.startsWith('/xd ') || lowerPrompt.startsWith('xd.')) {
    const priorityTools = ['desk_xd_command', 'desk_capabilities', 'desk_call_capability', 'desk_state'];
    return {
      id: 'xd-command',
  ...
  const priorityTools = ['desk_state', 'desk_active_context', 'desk_capabilities'];
  return {
    id: 'desk-general',
