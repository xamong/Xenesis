...
export async function importLegacyArray<T>(
  db: DatabaseSync,
  opts: {
    table: string;
    jsonPath: string;
    toRecord: (raw: unknown) => T | null;
    insertAll: (records: T[]) => void;
  },
...
export async function importLegacyMap<T>(
  db: DatabaseSync,
  opts: {
    table: string;
    jsonPath: string;
    entries: (parsed: unknown) => T[];
    insertAll: (records: T[]) => void;
  },
): Promise<{ imported: number; skipped: boolean }> {
  const parsed = await readJsonIfExists(opts.jsonPath);
  ...
  try {
    const count = (db.prepare(`SELECT COUNT(*) AS c FROM ${opts.table}`).get() as { c: number }).c;
    if (count === 0) {
      const records = opts.entries(parsed);
      opts.insertAll(records);
      imported = records.length;
    }
    db.exec('COMMIT');
