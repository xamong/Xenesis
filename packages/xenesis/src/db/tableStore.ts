...
export class TableStore<T> {
  ...
  private idColumn() {
    return this.spec.table === 'channel_sessions'
      ? 'key'
      : this.spec.table === 'plugins'
        ? 'path'
        : ['plan_sessions', 'worktree_sessions', 'team_sessions'].includes(this.spec.table)
          ? 'session_id'
          : this.spec.table === 'teams'
            ? 'team_name'
            : 'id';
  }

  get(id: string): T | undefined {
    const row = this.db.prepare(`SELECT data FROM ${this.spec.table} WHERE ${this.idColumn()} = ?`).get(id) as
      | { data: string }
      | undefined;
  ...
  private columnsAndValues(record: T) {
    ...
    const values: (string | number | null)[] = [
      ...
      ...this.spec.indexColumns.map((c) => derived[c] ?? null),
    ];
  ...
  upsert(record: T): void {
    ...
    const placeholders = cols.map(() => '?').join(', ');
    const updates = cols
      .filter((c) => c !== this.idColumn() && c !== 'rev')
      .map((c) => `${c}=excluded.${c}`)
      .concat('rev=' + this.spec.table + '.rev+1')
      .join(', ');
    this.db
      .prepare(
        `INSERT INTO ${this.spec.table} (${cols.join(', ')}) VALUES (${placeholders}) ON CONFLICT(${this.idColumn()}) DO UPDATE SET ${updates}`,
      )
      .run(...values);
  }
  ...
  updateOptimistic(id: string, mutate: (current: T) => T): T {
    for (let attempt = 0; attempt < MAX_OPTIMISTIC_RETRIES; attempt++) {
      const row = this.db.prepare(`SELECT data, rev FROM ${this.spec.table} WHERE ${this.idColumn()} = ?`).get(id) as
      ...
      const res = this.db
        .prepare(`UPDATE ${this.spec.table} SET ${setCols.join(', ')} WHERE ${this.idColumn()} = ? AND rev = ?`)
        .run(...setVals, id, row.rev);
