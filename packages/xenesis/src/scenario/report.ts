...
import { displayXenesisStatePath, loadConfig, type CliConfigOverrides, xenesisStatePath } from "../config/index.js";
import { closeAllDatabases } from "../db/database.js";
import { startGateway, type GatewayHandle, type GatewayRunCli } from "../gateway/index.js";
import type { AgentRunEvent } from "../core/events.js";
import { SqliteAgentTaskStore, TaskWorker, type AgentTask } from "../orchestration/index.js";
...
async function scenarioPlanWork(options: ScenarioRuntimeOptions, workspaceRoot: string, configPath: string) {
  const plan = await captureCli(options, scenarioArgv(
    workspaceRoot,
    configPath,
    "plan",
    "--print",
  ...
  const work = await captureCli(options, scenarioArgv(
    workspaceRoot,
    configPath,
    "work",
    "--print",
...
async function scenarioAgentLoopQuality(options: ScenarioRuntimeOptions, workspaceRoot: string, configPath: string) {
  ...
  const retry = await captureCli(options, scenarioArgv(
    workspaceRoot,
    configPath,
    "--print",
    "mock:tool:flaky_once:{}"
  ));
  ...
  await writeFile(
    ...
    [
      ...
      "0123456789",
      "ABCDEFGHIJKLMNOPQRSTUVWXYZ",
      "xenesis-agent-loop-quality"
    ].join("\n"),
    "utf8"
  );
  const longOutput = await captureCli(options, scenarioArgv(
    workspaceRoot,
    configPath,
    "--print",
    "mock:tool:read:{\"path\":\"long-output.txt\"}"
  ));
...
async function scenarioChatContext(options: ScenarioRuntimeOptions, workspaceRoot: string, configPath: string) {
  const run = await captureCli(
    options,
    scenarioArgv(workspaceRoot, configPath, "chat", "--print"),
    {
      stdin: Readable.from([
        "first question\n",
        "mock:messages\n",
...
async function scenarioMemoryContext(options: ScenarioRuntimeOptions, workspaceRoot: string, configPath: string) {
  const add = await captureCli(options, scenarioArgv(
    workspaceRoot,
    configPath,
    "memory",
    "add",
  ...
  const run = await captureCli(options, scenarioArgv(
    workspaceRoot,
    configPath,
    "--print",
    "mock:system agent"
...
async function scenarioTaskLifecycle(options: ScenarioRuntimeOptions, workspaceRoot: string, configPath: string) {
  const start = await captureCli(options, scenarioArgv(
    workspaceRoot,
    configPath,
    "tasks",
    "start",
...
async function scenarioGatewayIdeContext(options: RunScenarioSuiteOptions, workspaceRoot: string) {
  ...
  try {
    ...
    const content = body.events
      ?.filter((event): event is Extract<AgentRunEvent, { type: "assistant_message" }> => event.type === "assistant_message")
...
async function writeLongRunningHandoffConfig(
  workspaceRoot: string,
  options: { operationalFailures?: boolean } = {}
) {
  await writeFile(
    join(workspaceRoot, "xenesis.config.json"),
    JSON.stringify({
      ...
      maxTurns: 4,
      approvalMode: "auto",
      ...(options.operationalFailures === false
        ? { context: { operationalFailures: { enabled: false } } }
        : {}),
      extensions: {
        memory: { enabled: true, path: ".xenesis/memory.json" }
      }
...
async function scenarioLongRunningHandoff(options: RunScenarioSuiteOptions, workspaceRoot: string) {
  ...
  try {
    ...
    if (trace.diagnostics?.handoffTaskCount !== 4) throw new Error("trace diagnostics did not record handoffTaskCount=4");
...
async function scenarioHandoffWorkerExecution(options: RunScenarioSuiteOptions, workspaceRoot: string) {
  ...
  try {
    ...
    if (gatewayTasks.get("blocked")?.status !== "blocked") throw new Error("gateway tasks did not expose blocked status");
...
function renderScenarioLines(report: ScenarioReport, xenesisHome: string, reportPath: string) {
  ...
  return [
    ...
    ...report.scenarios.map((scenario) => scenario.status === "passed"
...
export function renderScenarioReportDetails(
  ...
  for (const scenario of report.scenarios) {
    lines.push(scenario.status === "passed"
      ? `scenario: ${scenario.name} passed`
      : `scenario: ${scenario.name} failed - ${scenario.message}`);
...
export async function runScenarioSuite(options: RunScenarioSuiteOptions): Promise<RunScenarioSuiteResult> {
  ...
  const config = await loadConfig({ cwd: options.cwd, configPath: options.configPath, env: options.env, cli: options.cli });
  ...
  try {
    ...
    const runScenarioWithDiagnostics = async (
      ...
      return await runScenario(
        name,
        () => fn({ ...scenarioOptions, diagnostics }),
        diagnostics
      );
    };
    scenarios = [
      await runScenarioWithDiagnostics("plan-work", (runtimeOptions) => scenarioPlanWork(runtimeOptions, scenarioWorkspace, scenarioConfigPath)),
      await runScenarioWithDiagnostics("agent-loop-quality", (runtimeOptions) => scenarioAgentLoopQuality(runtimeOptions, scenarioWorkspace, scenarioConfigPath)),
      await runScenarioWithDiagnostics("chat-context", (runtimeOptions) => scenarioChatContext(runtimeOptions, scenarioWorkspace, scenarioConfigPath)),
      await runScenarioWithDiagnostics("memory-context", (runtimeOptions) => scenarioMemoryContext(runtimeOptions, scenarioWorkspace, scenarioConfigPath)),
      await runScenarioWithDiagnostics("task-lifecycle", (runtimeOptions) => scenarioTaskLifecycle(runtimeOptions, scenarioWorkspace, scenarioConfigPath)),
      ...
      await runScenario("handoff-worker-execution", () => scenarioHandoffWorkerExecution(scenarioOptions, scenarioWorkspace))
    ];
  } finally {
    closeAllDatabases();
