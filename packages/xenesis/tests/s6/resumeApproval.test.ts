...
import { hasApprovalResolved, readSessionLog } from '../../src/sessions/history.js';
...
describe('hasApprovalResolved + pairing exclusion (S6)', () => {
  ...
  it('repairToolResultPairing excludes the pending toolCallId (no synthetic placeholder for it)', () => {
    ...
    const tc1Result = repaired.find((m) => m.role === 'tool' && (m as { toolCallId?: string }).toolCallId === 'tc1');
    ...
    const h = await seedLog([
      { type: 'user_message', sessionId: 'sess', timestamp: 't', message: { role: 'user', content: 'create a file' } },
      {
        ...
        message: {
          role: 'assistant',
          content: '',
          toolCalls: [{ id: 'tc1', name: 'write', input: { path: 'out.txt', content: 'hello' } }],
        },
      },
      {
        type: 'run_snapshot',
    ...
    const records = [
      { type: 'user_message', sessionId: 'sess', timestamp: 't', message: { role: 'user', content: 'create a file' } },
      {
        ...
        message: {
          role: 'assistant',
          content: '',
          toolCalls: [{ id: 'tc1', name: 'write', input: { path: 'out2.txt', content: 'hello2' } }],
        },
      },
      {
        type: 'run_snapshot',
          ...
          if (turn === 1) {
            return {
              message: {
                role: 'assistant',
                content: '',
                toolCalls: [{ id: 'bg1', name: 'echo', input: { cmd: 'ls' } }],
              },
              stopReason: 'tool_use',
            };
          }
