...
import type { IsolationOutcome, ProvisionedWorkspace, WorkspaceProvisioner } from '../../src/core/isolation/index.js';
...
describe('createPipelineTaskExecutor — paused -> blocked (S6)', () => {
  ...
  it('does NOT auto-deny: the executor never reports a failure/exit-code error for a pause', async () => {
    ...
    await expect(executor(makeTask(), { signal: new AbortController().signal })).resolves.toMatchObject({
      status: 'blocked',
    });
  });
