...
import { sanitizeMcpToolName, dedupeToolName } from "../../src/extensions/mcp.js";
...
describe("MCP tool-name safety", () => {
  ...
  it("dedupes collisions deterministically and stays <=64", () => {
    ...
    const a = dedupeToolName("mcp__s__tool", taken); taken.add(a);
    const b = dedupeToolName("mcp__s__tool", taken); taken.add(b);
    expect(a).not.toBe(b);
