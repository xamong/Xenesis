...
describe("config env interpolation and secret refs", () => {
  ...
  test("loadConfig interpolates file config without dropping final-only config fields", async () => {
    ...
    try {
      await writeFile(join(root, "xenesis.config.json"), JSON.stringify({
        model: "${MODEL_NAME}",
        context: {
          llmSummary: false,
  ...
  test("loadConfig preserves profile-layer MCP SecretRef and final extension fields", async () => {
    ...
    try {
      ...
      await writeFile(join(xenesisHome, "profiles.json"), JSON.stringify({
        active: "work",
        profiles: {
          work: {
            hooks: { enabled: false },
  ...
  test("resolves env, file, and exec secret references", async () => {
    ...
    try {
      ...
      expect(await resolveSecretRef({ source: "env", id: "API_KEY" }, { API_KEY: "env-secret" } as NodeJS.ProcessEnv)).toBe("env-secret");
      expect(await resolveSecretRef({ source: "file", id: secretPath }, {} as NodeJS.ProcessEnv)).toBe("file-secret");
      expect(await resolveSecretRef({ source: "exec", id: `${quotedExecPath(process.execPath)} --version` }, {} as NodeJS.ProcessEnv))
        .toMatch(/^v\d+\.\d+\.\d+/);

      await expect(resolveSecretRef({ source: "env", id: "MISSING" }, {} as NodeJS.ProcessEnv)).rejects.toThrow(/MISSING/);
      await expect(resolveSecretRef({ source: "exec", id: "   " }, {} as NodeJS.ProcessEnv)).rejects.toThrow(/must not be empty/);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
