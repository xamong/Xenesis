...
import { contextSchema, loadConfig } from '../../src/config/loadConfig.js';
import { type ContextConfig, defaultConfig } from '../../src/config/types.js';
...
describe('S2 ContextConfig keys', () => {
  ...
  it('persists compactTokenThresholdRatio as a number that survives reload', async () => {
    ...
    try {
      ...
      const result = await configTool.run({ setting: 'context.compactTokenThresholdRatio', value: '0.7' }, context);
  ...
  it('rejects an out-of-range compactTokenThresholdRatio without writing', async () => {
    ...
    try {
      const context = makeContext(root);
      const result = await configTool.run({ setting: 'context.compactTokenThresholdRatio', value: '5' }, context);
