...
const img = (name: string) => ({ kind: "image" as const, name, mimeType: "image/png", dataUrl: "data:image/png;base64,AAA" });
const toolImg = (id: string, name: string): AgentMessage =>
  ({ role: "tool", toolCallId: id, name: "screenshot", content: "shot", attachments: [img(name)] });
...
describe("stripStaleImageAttachments", () => {
  ...
  it("never mutates the input objects (clone proof)", () => {
    ...
    const msgs: AgentMessage[] = [original, { role: "user", content: "1" }, { role: "user", content: "2" }, { role: "user", content: "3" }, toolImg("b", "new.png")];
  ...
  it("returns the same array contents (count/order preserved) and is a no-op with no images", () => {
    const msgs: AgentMessage[] = [{ role: "user", content: "a" }, { role: "assistant", content: "b" }];
    const out = stripStaleImageAttachments(msgs, { keepRecentTurns: 3 });
