...
import { buildSummarizationMessages, createLlmSummarizer, SUMMARIZER_MIN_CONTEXT } from "../../src/core/context/compaction/llmSummarizer.js";
...
describe("createLlmSummarizer", () => {
  it("calls complete with the built messages + maxTokens and returns its text", async () => {
    ...
    const summarize = createLlmSummarizer({
      complete: async (messages, maxTokens) => { seenMax = maxTokens; seenLen = messages.length; return "LLM SUMMARY"; },
      maxTokens: 4096
    });
