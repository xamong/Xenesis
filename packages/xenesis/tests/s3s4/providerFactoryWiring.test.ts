...
describe('createProvider <-> registerProviderFactory wiring (Spec section 5)', () => {
  ...
  it('does not throw resolving settings for an external provider with no preset', () => {
    ...
    expect(() => createProvider(configFor('edge-gateway'), {} as NodeJS.ProcessEnv)).not.toThrow();
  });
  ...
  it('blocks the built-in mock provider unless the explicit test gate is enabled', () => {
    expect(() => createProvider(configFor('mock'), {} as NodeJS.ProcessEnv)).toThrow(
      /mock provider is blocked|provider mock is blocked/i,
    );
  });

  it('allows the built-in mock provider when the explicit test gate is enabled', () => {
    const provider = createProvider(configFor('mock'), {
  ...
  it('does not use configured fallbacks when the primary provider is missing credentials', async () => {
    ...
    expect(() =>
      createProvider(configWithFallbacks('openai', [{ provider: 'codex-app-server' }]), {
  ...
  it('resolves auto with CODEX_HOME auth to codex-app-server', async () => {
    ...
    const provider = createProvider(configFor('auto'), { CODEX_HOME: codexHome } as NodeJS.ProcessEnv);
  ...
  it('xenesis init defaults to the shared auto provider instead of openai', async () => {
    ...
    expect(source).toMatch(
      /env\.XENESIS_PROVIDER \? parseProviderName\(env\.XENESIS_PROVIDER\) : defaultConfig\.provider/,
    );
  ...
  it('readonly tool filtering is independent of the configured provider name', async () => {
    ...
    const coreSource = await readFile(new URL('../../src/core/AgentRuntimeFactory.ts', import.meta.url), 'utf8');
    const cliSource = await readFile(new URL('../../src/cli/main.ts', import.meta.url), 'utf8');
  ...
  it('runAgentPipeline disposes repair runners and only disposes the main runner when requested', async () => {
    const source = await readFile(new URL('../../src/core/AgentRunPipeline.ts', import.meta.url), 'utf8');

    expect(source).toMatch(
      /finally\s*\{\s*if \(options\.disposeRunner === true\)\s*\{\s*await disposePipelineRunner\(built\.runner\);\s*\}\s*\}/,
    );
