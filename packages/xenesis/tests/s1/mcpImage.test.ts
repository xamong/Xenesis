...
import { createMcpTools, type McpToolClient, type McpToolResult, splitMcpContent } from '../../src/extensions/mcp.js';
...
describe('splitMcpContent', () => {
  ...
  it('guards malformed image block — missing data — no throw', () => {
    const { text, attachments } = splitMcpContent([{ type: 'image', mimeType: 'image/png' }]);
  ...
  it('drops unsupported MIME types', () => {
    const { attachments } = splitMcpContent([{ type: 'image', data: 'AAA', mimeType: 'image/svg+xml' }]);
    expect(attachments).toHaveLength(0);
  });
