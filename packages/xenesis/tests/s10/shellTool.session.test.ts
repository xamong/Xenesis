...
describe("createShellTool persistence + lifecycle", () => {
  it("persists cwd across two run() calls in the same session, then cleanupSession disposes", async () => {
    ...
    await tool.run(
      { command: isWin ? "$env:T='1'" : "export T=1", timeoutMs: 30000 } as never,
      ctx(root, "S")
    );
  ...
  it("persistent:false uses the stateless path (no session persistence)", async () => {
    ...
    await tool.run(
      { command: isWin ? "$env:T2='x'" : "export T2=x", timeoutMs: 30000 } as never,
      ctx(root, "S2")
    );
    const r = await tool.run(
  ...
  it("cleanupSession kills the tracked child PID — no live process after dispose", async () => {
    ...
    await tool.run(
      { command: isWin ? "Write-Output up" : "echo up", timeoutMs: 30000 } as never,
      ctx(root, "KILL")
    );
    ...
    expect(
      events.some((e) => e.phase === "dispose" && e.sessionId === "KILL" && e.pid === pid)
    ).toBe(true);
