...
describe("plugin manifest providers slot", () => {
  it("parses a providers entry", async () => {
    const d = await manifestDir({ name: "p", providers: [{ name: "myllm", entry: "./prov.mjs", exportName: "factory", capabilities: { supportsTools: true, requiresApiKey: false, transport: "http-streaming", streaming: true, persistentSession: false } }] });
  ...
  it("rejects a malformed capabilities shape", async () => {
    const d = await manifestDir({ name: "p", providers: [{ name: "x", entry: "./e.mjs", exportName: "f", capabilities: { supportsTools: "yes" } }] });
