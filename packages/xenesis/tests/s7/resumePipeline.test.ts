...
describe("resumeAgentPipeline", () => {
  it("rehydrates history + restores the run_snapshot (turns != 0) and continues with the same sessionId via mock provider", async () => {
    const h = await seedLog([
      ...
      { type: "assistant_message", sessionId: "sess", timestamp: "t", message: { role: "assistant", content: "working" } },
      fullSnapshot(2)
    ]);
    ...
    const newSnapshots = recs.filter(
      (r): r is typeof r & { state: { turns: number } } => r.type === "run_snapshot"
    );
