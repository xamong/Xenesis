...
async function home() { return await mkdtemp(join(tmpdir(), "s7-")); }
...
describe("readSessionLog truncation tolerance", () => {
  it("skips a truncated trailing line instead of throwing", async () => {
    ...
    await writeFile(path, JSON.stringify({ type: "user_message", sessionId: "sess", timestamp: "t", message: { role: "user", content: "ok" } }) + "\n", "utf8");
    await appendFile(path, '{"type":"assistant_message","sessionId":"sess","timestamp":"t","message":{"role":"assist', "utf8"); // truncated, no newline
    const recs = await readSessionLog(h, "sess");
