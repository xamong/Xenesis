...
describe('lazy backfill', () => {
  it('embeds pre-existing NULL-embedding rows in the background', async () => {
    ...
    db.prepare('INSERT INTO memory (id, priority, updated_at, rev, data) VALUES (?,?,?,?,?)').run(
      ...
      0,
      JSON.stringify({ id: 'old', text: 'pre-existing memory row', tags: [], updatedAt: '2026-01-01T00:00:00.000Z' }),
    );
    ...
    for (
      let i = 0;
      i < 10 &&
      (db.prepare('SELECT embedding FROM memory WHERE id=?').get('old') as { embedding: unknown }).embedding == null;
      i += 1
    )
      await tick();
...
describe('lazy backfill termination on embedder failure', () => {
  it('does not re-select a persistently failing row in a tight loop (bounded embed calls)', async () => {
    ...
    db.prepare('INSERT INTO memory (id, priority, updated_at, rev, data) VALUES (?,?,?,?,?)').run(
      ...
      '2026-01-01T00:00:00.000Z',
      0,
      JSON.stringify({ id: 'a-fails', text: 'boom row', tags: [], updatedAt: '2026-01-01T00:00:00.000Z' }),
    );
    db.prepare('INSERT INTO memory (id, priority, updated_at, rev, data) VALUES (?,?,?,?,?)').run(
      'b-ok',
      0,
      '2026-01-01T00:00:00.000Z',
      0,
      JSON.stringify({ id: 'b-ok', text: 'good row', tags: [], updatedAt: '2026-01-01T00:00:00.000Z' }),
    );
    ...
    for (
      let i = 0;
      i < 20 &&
      (db.prepare('SELECT embedding FROM memory WHERE id=?').get('b-ok') as { embedding: unknown }).embedding == null;
      i += 1
    )
      await tick();
