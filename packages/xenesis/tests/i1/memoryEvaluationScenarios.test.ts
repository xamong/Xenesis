...
describe('memory evaluation scenarios', () => {
  it('exposes fixed memory evaluation scenarios outside the broad default suite', () => {
    ...
    expect(defaultMemoryEvaluationScenarios.every((scenario) => scenario.fixture === 'memory-evaluation-project')).toBe(
      true,
    );
    ...
    const result = evaluateCapabilityRun({
      scenario: recall,
      exitCode: 0,
      stdout: ['tool: memory', 'memory event: recall', 'evidence id: evidence-format', '짧고 실행 중심'].join('\n'),
      stderr: '',
      durationMs: 42,
    });
