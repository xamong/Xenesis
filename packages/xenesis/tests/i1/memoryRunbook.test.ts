...
describe("procedural runbook memory", () => {
  it("accepts manual low-risk draft runbooks as active procedure memory", async () => {
    ...
    const result = await ledger.write(createRunbookMemoryInput({
  ...
  it("updates runbooks through temporal supersede history", async () => {
    ...
    const supersede = await ledger.supersedeRecord("runbook-research-v1", createRunbookMemoryInput({
    ...
    expect((await ledger.history({ memoryId: "runbook-research-v1" })).map((event) => event.type)).toContain("superseded");
  });
  ...
  it("does not allow same-id runbook updates to overwrite temporal history", async () => {
    ...
    await expect(ledger.write(createRunbookMemoryInput({
      id: "runbook-same-id",
      runbook: normalizeMemoryRunbook({
        trigger: "리서치",
        steps: ["논문 검색", "벤치마크 확인"],
    ...
    }), userTrustedAt("2026-05-01T00:00:00.000Z"))).rejects.toThrow(/supersede/i);
  ...
  it("rejects invalid runbooks before persistence", async () => {
    ...
    }, userTrustedAt())).rejects.toThrow(/runbook/i);
