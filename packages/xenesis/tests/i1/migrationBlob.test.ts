...
describe("v4 embedding migration + BLOB round-trip", () => {
  it("(a) fresh DB: memory table has an embedding column and round-trips a Float32Array", async () => {
    ...
    db.prepare("INSERT INTO memory (id, priority, updated_at, rev, data) VALUES (?,?,?,?,?)")
      .run("m1", 0, "2026-01-01T00:00:00.000Z", 0, JSON.stringify({ id: "m1", text: "x", tags: [], updatedAt: "2026-01-01T00:00:00.000Z" }));
  ...
  it("(b) existing v3 DB with rows: migrating to v4 adds the column and existing rows get NULL", async () => {
    ...
    db.prepare("INSERT INTO memory (id, priority, updated_at, rev, data) VALUES (?,?,?,?,?)")
      .run("old-1", 0, "2025-12-31T00:00:00.000Z", 0, JSON.stringify({ id: "old-1", text: "legacy", tags: [], updatedAt: "2025-12-31T00:00:00.000Z" }));
    ...
    expect(memoryColumns(db).includes("embedding")).toBe(true);
    const row = db.prepare("SELECT id, embedding FROM memory WHERE id = ?").get("old-1") as { id: string; embedding: unknown };
