...
import { LocalExecutionBackend, LOCAL_BACKEND } from "../../src/core/isolation/executionBackend.js";
...
describe("LocalExecutionBackend", () => {
  ...
  it("run delegates to runCommand (real trivial command)", async () => {
    const r = await LOCAL_BACKEND.run({ command: isWin ? "Write-Output ok" : "echo ok", cwd: process.cwd(), timeoutMs: 30000 });
  ...
  it("runArgs delegates to runCommandArgs (real trivial command)", async () => {
    ...
    const r = await LOCAL_BACKEND.runArgs({ command: process.execPath, args: ["-e", "process.stdout.write('ok')"], cwd: process.cwd(), timeoutMs: 30000 });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("ok");
  }, 30000);
});
