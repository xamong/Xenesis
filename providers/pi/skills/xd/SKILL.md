...
Use this skill when the user invokes `/xd`, mentions Xenesis Desk control, asks to operate Xenesis Desk from Pi, Codex, Claude Code, Hermes, Telegram, Discord, Slack, or wants terminal/file/XCON/panel/stash control through the Xenesis Desk MCP bridge or the Hermes `xenesis_desk_gateway` plugin.
...
- The user wants Pi, Codex, or Claude Code to drive Xenesis Desk through MCP, including terminal, safe file write, XCON, panel, diagnostics, and Playwright tasks.
...
|---|---|
| Pi, Codex, or Claude Code with Xenesis Desk MCP tools | Use the available `xenesis_desk_*` MCP tools directly. |
| Pi, Codex, or Claude Code managing Hermes-only stash/mobile features | Open Xenesis Bot or Hermes Stash Operations through MCP, then route commands through the Hermes gateway/plugin. |
...
Do not assume Hermes `/xd` exists inside Pi, Codex, or Claude Code. In Pi, Codex, and Claude Code, `/xd` is this skill's callsign; actual execution should be via MCP tools unless the task explicitly targets the Hermes gateway/plugin.
...
- Keep Hermes plugin `/xd` and Pi/Codex/Claude skill `/xd` conceptually separate.
