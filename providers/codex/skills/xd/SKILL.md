...
Use this skill when the user invokes `/xd`, mentions Xenesis Desk control, asks to operate Xenesis Desk from Codex, Claude Code, Hermes, Telegram, Discord, Slack, or wants terminal/file/XCON/panel/stash control through the Xenesis Desk MCP bridge or the Hermes `xenesis_desk_gateway` plugin.

The short callsign is `xd`. In hosts that map skills to slash commands, invoke this as `/xd`. In hosts that use skill variables, invoke it as `$xd`.

## When to Use

- The user asks for `/xd status`, `/xd run`, `/xd tail`, `/xd open`, `/xd xcon`, `/xd panels`, `/xd actions`, `/xd stash`, or similar Xenesis Desk control.
- The user wants Codex or Claude Code to drive Xenesis Desk through MCP, including terminal, safe file write, XCON, panel, diagnostics, and Playwright tasks.
...
|---|---|
| Codex or Claude Code with Xenesis Desk MCP tools | Use the available `xenesis_desk_*` MCP tools directly. |
| Codex or Claude Code managing Hermes-only stash/mobile features | Open Xenesis Bot or Hermes Stash Operations through MCP, then route commands through the Hermes gateway/plugin. |
...
Do not assume Hermes `/xd` exists inside Codex or Claude Code. In Codex and Claude Code, `/xd` is this skill's callsign; actual execution should be via MCP tools unless the task explicitly targets the Hermes gateway/plugin.
...
- Keep Hermes plugin `/xd` and Codex/Claude skill `/xd` conceptually separate.
